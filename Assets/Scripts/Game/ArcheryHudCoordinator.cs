...
{
    /// <summary>
    /// 내 몸이 생기면 Archery 인게임 화면(조작 패드 + 디버그 HUD)을 연다.
    /// 엔티티 생성과 화면 띄우기를 분리한다 — 화면 교체는 "큰 흐름"이라 코디네이터 책임
    /// (아키텍처 가이드라인 "흐름의 경계"). <see cref="SkydiveHudCoordinator"/>와 같은 짝이다.
    /// </summary>
    public class ArcheryHudCoordinator : MessageHandlerBase
    {
