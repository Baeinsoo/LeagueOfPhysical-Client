...
namespace LOP.MapTools
{
    ...
    public static class BotPilot
    {
            ...
            ExactFreeSpaceProbe isFree)
        {
            ...
            //
            //  ⚠️ <b>"더 높이 겨냥하면 낫지 않나"는 재 봤고 전부 나빴다 (2026-09-14).</b> 봇이
            //  실패하는 직접 원인은 "안 오른다"로 보였다 — 비행 1675틱 중 91%가 <i>누를 뜻조차
            //  없던</i> 틱이고, 관문에 빠르게 떨어지며(vy −17.6) 도착해 죽는다(통과한 비행은
            //  −9.2~−5.0). 그래서 겨냥 높이를 lowNear 말고 다른 데로 옮긴 후보 넷을 같은 자
            //  (18줄 높이 훑기의 완주 수, 지금 <b>2</b>)로 채점했다:
            //      지금(틈 바닥+반지름)       완주 2 · 네 스폰 366.5 ×4        · 누를 뜻 없음 91%
            //      틈 위끝−(아치+몸)          완주 0 · 86.2/46.7/164.7/46.7   · 58%
            //      틈 가운데                  완주 2 · 82.7/86.2/329.5/369.5  · 81%
            //      틈 바닥+아치/2             완주 0 · 82.7/369.5/329.6/82.7  · 92%
            //      천장 바로 아래(가드에 위임) 완주 0 · 46.7/14.7/46.7/14.7    · 23%
            //  <b>2를 넘은 후보가 없어 아무것도 넣지 않았다.</b> 겨냥을 천장 쪽으로 옮기면
            //  "누를 뜻 없음"은 실제로 23%까지 떨어지는데(= 규칙은 의도대로 작동한다) 완주는
            //  오히려 0이 된다 — 높이 겨냥할수록 천장 가드가 막는 틱이 늘고(5% → 60%) 봇이
            //  코스 초반 막다른 높은 통로로 일찍 올라가 갇힌다. 즉 <b>막는 것은 "얼마나 높이
            //  겨냥하나"가 아니라 "어느 통로를 타나"</b>이고, 그건 한 틱짜리 겨냥 규칙으로는
            //  표현이 안 된다(<see cref="BotRollout"/>의 같은 결론과 맞물린다).
