...
namespace LOP.MapTools
{
    ...
    public static class PlayabilityReport
    {
                                            ...
                                            float startX, float finishX)
        {
            ...
            //  차이가 정확히 0이면 두 모델의 <b>높이 산술이 같다</b>는 측정 결과다. 그 경우
            //  아래 외삽("같은 비율로 끝까지 가면 약 0.0m")은 뜻이 없을 뿐 아니라 "편향이
            //  쌓이는 중"으로 읽혀 고칠 곳을 잘못 짚게 한다 — 대신 무엇이 남았는지를 적는다.
        ...
        //  maxDistance로 자르며, FlappyRace.unity에서 위치 (0,2,-30) · maxDistance 32라
        //  결과가 30m다. (FlappyCameraFollow는 옛 프로토타입 전용이고 이 값과 무관하다 —
        //  전에 그 죽은 파일을 출처로 적어 뒀다가, 09-18 물리 보정이 존재하지 않는 카메라를
        //  기준으로 계산되는 사고가 났다.)
