...
namespace LOP
{
    public class RoomEventBus : MonoSingleton<RoomEventBus>, IEventBus
    {
        ...
        void IEventBus.Subscribe<T>(Action<T> handler)
        {
            if (handlerMap.TryGetValue(typeof(T), out var baseEventHandler))
            {
                ...
                {
                    Debug.LogWarning($"[RoomEventBus] EventHandler for {typeof(T).Name} is of a different type.");
                }
            }
            else
            {
        ...
        void IEventBus.Unsubscribe<T>(Action<T> handler)
        {
            ...
            {
                Debug.LogWarning($"[RoomEventBus] No handlers registered for {typeof(T).Name}");
            }
        }
