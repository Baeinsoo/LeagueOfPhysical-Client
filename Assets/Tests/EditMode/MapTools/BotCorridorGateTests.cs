...
namespace LOP.MapTools.Tests
{
    ...
    ///
    /// <para><b>숫자가 둘인 이유</b>: 회랑 폭은 <i>천장 쪽</i> 보수성만 잡는다(아래 "게이트의
    /// 반대쪽 절반" 참고 — 바닥 문턱을 ±0.05 움직여도 회랑 숫자는 안 변한다). 그래서 바닥 쪽
    /// 보수성을 잡는 숫자를 하나 더 둔다. 둘을 함께 두어야 봇이 어느 쪽으로 흔들려도 여기서
    /// 걸린다.</para>
    /// </summary>
    public class BotCorridorGateTests
    {
        ...
        //  ── 게이트의 반대쪽 절반 (바닥 규칙) ──
        //  위 회랑 숫자는 <b>천장 쪽 보수성만</b> 잰다. 실측으로 확인했다: 바닥 문턱(safeFloor)에
        //  ±0.05를 줘도 회랑 숫자는 4.4750에서 1mm도 안 움직인다. 구조적으로 그렇다 — 폭이
        //  4.475m인 회랑에서 아치(4.012)가 들어가는 높이는 y ≤ 0.463뿐이라, "언제 누르고
        //  싶은가"와 무관하게 <b>언제 누를 수 있는가</b>가 판을 지배한다. 그래서 바닥 쪽을
        //  잡는 숫자를 하나 더 못박는다. 이것 없이는 게이트가 반쪽이다(겁쟁이 봇이 초록으로
        //  빠져나간다).
        ...
        [Test]
        public void 아치보다_좁은_회랑은_어느_높이에서_출발해도_못_버틴다()
        {
            //  하니스가 무엇이든 초록으로 만드는 물건이 아님을 보이는 아래쪽 짝. 3m는 날갯짓
            //  한 번의 아치(4.012)보다도 좁아, 봇이 어떻게 조종하든 누르는 순간 천장을 뚫는다 —
            //  즉 물리적으로 불가능한 폭이다.
