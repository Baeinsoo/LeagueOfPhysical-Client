...
/// <summary>
/// <see cref="SkydiveCourseBuilder"/>는 <c>SpreadWindLag</c>·<c>DiveWindLag</c>를 상수로 박아 둔다
/// (이 파일의 다른 검사 기준값들과 같은 컨벤션 — 기준이 데이터를 따라 조용히 움직이면 검사가
/// 아니게 된다). 하지만 아무도 <c>TbSkydiveConfig</c>와 같은 값인지 확인하지 않으면, 마스터데이터를
/// 만졌을 때 이 상수가 조용히 낡아 굽기 전 검사가 더 이상 실제 코스를 재지 않게 된다.
///
