...
| 🟢 | **내 엔티티 id가 두 군데에 있다** — `gameDataStore.userEntityId`와 `playerContext.entityId`가 같은 값을 들고 **같은 스코프**(`RoomLifetimeScope`)에 산다. 차이는 *언제부터 유효한가* 하나뿐이다 — 앞의 것은 `GameInfoToC`를 받은 순간, 뒤의 것은 *내 새가 실제로 만들어진 뒤* 크리에이터가 채운다. 그래서 스폰 시점 코드(`EntityBinder` + 크리에이터 4개)는 앞의 것을, 나머지는 뒤의 것을 쓴다(읽는 파일 10개 vs **21개**). `GameInfoToC`를 받을 때 `playerContext.entityId`도 채우면 `userEntityId`는 지울 수 있다. **위험은 21곳 중 일부가 null을 "내 엔티티가 아직 없다"는 신호로 쓴다는 것** — 채우는 시점을 앞당기면 그 의미가 "id를 안다"로 조용히 바뀐다(`FlappyAutoFlapSystem:94`가 그 예, 다행히 뒤에 레지스트리 가드가 하나 더 있다). 전수 확인이 이 슬라이스의 본체다 | 중간 | 아니오 |
