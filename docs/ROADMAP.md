...
- **▶ 다음 (스펙 완료, 플랜 대기) — 엔티티 매니저 → `ActorRegistry` + `EntitySpawner` 분리**: `LOPEntityManager`의 두 책임(actor 뷰 명부 / 출생·사망 조율)을 리액티브 뷰 리졸버 표준대로 분리. `EntitySpawner`(데이터 수명, actor 미참조)+`EntityRegistry` ↔ `EntityBinder`(뷰 수명)+`ActorRegistry`(신규 순수 인덱스). `EntityCreated` 페이로드 `LOPActor`→`entityId` flip, GameFramework 옛 인터페이스 3종(`IEntityManager`/`IEntityFactory`/`IEntityCreator`) 삭제, 서버 `EntityViewSpawner`→`EntityBinder` 이름 통일. `LOPActor`는 클·서 각자 유지(뷰 접근자 diverge 정당). 이게 아래 "의도적 보류"의 `entityMap`→스포너 이전 + `IEntityManager` 규격 해체를 실현. **spec 완료·커밋**(`specs/2026-07-19-entity-manager-actor-registry-split-design`, 브랜치 `feature/entity-manager-actor-registry-split`). **다음 단계 = writing-plans → subagent-driven(클·서 양쪽).**
- **의도적 보류(별도 슬라이스, 필요 시)**: **PhysicsBody 순수 포트화(Y)** (`[[physicsbody-port-purity-deferred]]`, 코어는 이미 IMotionBridge로 격리). (`entityMap`→스포너 이전 / `IEntityManager` 규격 해체는 위 "다음" 슬라이스가 흡수.)
- **비차단 후속(기회 정리)**: register→PhysicsBody 순서 불변 주석 · S5b Task1 rationale 주석 복원. (~~`CharacterCreationDataCreator` param `lopEntity`→`actor`~~ ✅ 07-19 로직 분리 슬라이스에서 이미 `worldEntity`로 리네임됨 — stale 노트 제거.)
