...
전부 x = −2로 같아서 예산도 같다. 이 전제는 이제 코드에 적혀 있고(`FlappyMapPlayabilityCheck.cs:162-164`),
스폰들의 x가 서로 다르면 보고서가 경고를 찍는다(`:173-188`). 남은 건 경고가 아니라 계산 자체다
— 다음 슬라이스가 ③도 자리별로 계산해야 한다.
