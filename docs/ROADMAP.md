...
`BoxCollider.center`를 반대로 보정해 월드 bounds를 비트 단위로 그대로 유지한다. 씬을 다시
열고 `Physics.SyncTransforms()` 후 독립 재측정한 결과, 정렬 전후 콜라이더 월드 z가 정확히
`[-1.25, +1.25]`로 동일했다(`InverseTransformVector` 보정이 부모 배율·회전을 정확히
되돌린다). Art 커밋 `8c332be`가 최종 형태다.
...
| 확인 | 기대값 | 실측값 | 출처 |
|---|---|---|---|
| 네 자리 판정 | ✅ ✅ ✅ ✅ | ✅ ✅ ✅ ✅ | `Logs/FlappyMapCheck.txt`(09-16 09:18) |
| 날갯짓 수 | 105 / 110 / 102 / 102 | 105 / 110 / 102 / 102 | 위와 동일 |
| 회전 여유 | 0.15 / 0.10 / 0.10 / 0.21m | 0.15 / 0.10 / 0.10 / 0.21m | 위와 동일 |
| 최소 여유 | 0.03 / 0.03 / 0.05 / 0.04m | 0.03 / 0.03 / 0.05 / 0.04m — **재측정 아님** | `Logs/FlappyProvenPaths.txt`(09-15 00:16, 정렬 전 씬 — 아래 참고) |
| 🎥 시각 정직성 | 아래 두 줄과 글자 단위 일치 | 일치 | `Logs/FlappyMapCheck.txt`(09-16 09:18) |
...
`Logs/FlappyMapCheck.txt`(09-16 09:18)는 정렬 완료 상태(Art `8c332be`)에서 돌린 것이다 —
그 뒤에 온 `f4b462ed`는 분류 규칙을 커널 한 곳으로 옮긴 **코드·리포트 전용** 커밋이라 씬을
다시 안 건드렸고(`git -C Assets/Art status` 빈 출력으로 확인됨), 더 엄격해진 분류기로 다시
센 결과도 `Wall=119 · RenderOnly=13 · ColliderElsewhere=0`로 동일해 이 리포트가 여전히
현재 상태를 대표한다.

**"최소 여유" 행만 재측정하지 못했다.** 이 숫자는 맵 검사가 아니라 별도 도구
(`FlappyMapPlayabilityCheck.ShowProvenPathsHeadless`, 스폰 4자리에 약 65초)가 내는 값인데,
이 기록을 쓰는 시점에 에디터가 다른 대기 중인 잡으로 계속 바빠 새로 돌리지 못했다. 위 표의
실측값은 **정렬 전(09-15 00:16) 씬에서 잰 값**이며, 정렬 뒤 다시 재지 않았다. 다만 이 행의
중립성도 콜라이더 불변(위 "왜 중요한가")에서 따라 나온다 — 클린런 탐색·재생이 보는 것은
콜라이더뿐이고 콜라이더가 한 비트도 안 바뀌었으므로, 다시 재도 같은 값이 나와야 한다. 그래도
**직접 잰 값은 아니므로** 이 행만 확인 대기로 남긴다.
