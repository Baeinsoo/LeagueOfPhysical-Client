...
| 🟢 | **관전·나가기 UI** — 완주해도 탈락해도 화면에 문구만 뜨고 **할 수 있는 게 없다.** 완주자는 결승선 너머에 멈춰 서 있고 탈락자는 카메라가 자동으로 남은 꼴찌를 따라갈 뿐, 보고 싶은 사람을 고르거나 판을 떠날 방법이 없다. 두 상태가 필요로 하는 것이 같으므로 (다른 캐릭 보기 + 나가기) 한 부품으로 만들어 `RaceFinishView`·`RaceEliminatedView`가 함께 쓴다. 카메라 타깃 교체는 이미 `FlappyWatchTarget`·`CameraController`에 있고 남은 건 고르는 화면이다 | 중간 | 예 |
