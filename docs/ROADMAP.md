...
> 남은 것은 위치 TTL·해석 일원화·push·`locationDetail` 타입 강화다.
...
1. ~~**매치 종료 시 위치 정리**~~ ✅ **완료(08-15)** — 아래 절.
2. **게임서버가 스스로 종료하기** (신규, 08-15 업계 표준 대조에서 나옴) — 지금 게임서버는 **자기를
   끝내지 않고** 백엔드가 파드를 지워줘야만 내려간다. 그래서 백엔드가 죽어 있으면 파드와 hostPort가
   잡힌 채 남는다(② 검증에서 실물 관측). Agones 표준은 게임서버가 `SDK.Shutdown()` 후 **스스로 빠지는**
   모양이다. 통보를 끝낸 뒤 프로세스를 종료하면 파드·포트가 즉시 반납되고, 하트비트를 명시적으로 끌
   필요도 없어진다.
3. **위치 TTL** — 클라가 죽으면 그 사람은 영원히 그 상태다. (단 방-근거 경로는 08-15에 닫혔다 —
   방이 죽으면 위치도 죽는다.)
4. **클라 해석 일원화** — 지금 깨지는 건 없다. 위치 해석 *규칙*이 실제로 바뀔 때 같이 하는 게 값이 크다.
5. **`locationDetail` 타입 강화 / push** — 계약 변경이라 클·서 동시 작업.
