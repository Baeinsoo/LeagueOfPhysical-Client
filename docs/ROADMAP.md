...
2. **패키지 레포 CI 구멍** — GameFramework·Shared·MasterData에 푸시해도 CI가 뜨지 않는다.
   시뮬 공유 코드가 거기 있어 실제로 아픈 자리다. 원격으로 깨우려면 PAT이 필요하다.
3. **진짜 앱 아이콘** — 지금은 18KB 임시 이미지다.
4. Task 6 실지 확인 — 콘텐츠를 눈에 띄게 하나 바꿔 **앱 재설치 없이** 폰에 반영되는지.
5. 시험 수가 `-buildTarget` 유무로 1960 / 1957로 갈린다. 3개가 어느 것인지 확인 안 했다.
