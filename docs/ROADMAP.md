...
### 🟠 남은 별건 — 매치 시작 직후 입력 폐기 (미착수)

`pruneTot`이 회차마다 **0 · 3 · 8 · 11**로 들쭉날쭉하다. 폐기가 많은 회차엔 초반에 서버가
안 움직여(`srvPos=0`, `srvVel=0`) 클라만 걸었고, 4cm 스냅백이 생긴다 — 서버가 권위이므로
**보정 자체는 옳고**, 스냅백은 *버려진 입력의 대가*다. 즉 고칠 곳은 보정이 아니라 폐기 쪽이다.

`PruneBefore`는 서버가 이미 지나친 틱의 입력을 버린다. 시작 직후 우리 입력이 왜 지각하는지는
**아직 확인 안 했다**(서버 로그가 필요한데 파드 이미지라 재빌드가 비싸다). 클라 쪽에서 볼 수 있는
것은 `[HudDump]`의 `pruneTot`/`seqGapTot`뿐이다.

### 같이 확인된 별건 — 동시 스폰 히칭

같은 측정에서 적 10마리 동시 스폰 시 `fps` 60→53, `frameMs` 18.7, `snapGapMax` 121ms,
`pruneTot`/`seqGapTot` 11이 함께 관측됐다(체감 "살짝 끊김"). **`corrections`는 0** — 입력
파이프라인 3층이 받아내 위치는 안 어긋난다. 즉 보정 문제가 아니라 **순수 부하**다.
