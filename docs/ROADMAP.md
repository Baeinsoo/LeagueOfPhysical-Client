...
`SequenceBuffer.FirstRecordedTick`(GameFramework)로 조회 실패의 두 원인을 가른다 —
**① 참가 전 틱**(서버 상태가 내 예측을 반증하지 못함 → 무시) vs **② 링 밖으로 밀려난 틱**
(진짜로 뒤처짐 → 종전대로 복원). `Reconciler`는 ①이면 스냅을 그냥 버린다.

**측정 결과:** `reconMax` `0.040~0.080` → **`0.000`** · `corrections` `5~9` → **`0`** ·
`[ReconSpike]` 첫 틱 1건 → **0건**. EditMode 454/454(신규 5).
