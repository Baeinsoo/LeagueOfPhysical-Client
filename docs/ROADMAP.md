...
- **의도적 보류(별도 슬라이스, 필요 시)**: **PhysicsBody 순수 포트화(Y)** (`[[physicsbody-port-purity-deferred]]`, 코어는 이미 IMotionBridge로 격리). (`entityMap`→스포너 이전 / `IEntityManager` 규격 해체는 아래 Done 원장의 "엔티티 매니저 분리" 슬라이스에서 완료.)
