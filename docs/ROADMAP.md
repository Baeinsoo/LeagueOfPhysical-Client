...
## 🟠 플레이어 신원 — 표시 이름 + 태그 (슬라이스 1 완료, 2026-08-22)

**한 문장:** `Guest-<uuid>` 대신 사람이 읽는 이름을 주되, **이름 중복은 허용하고 신원은 태그가 가른다.**
슬라이스 1(스키마 + 태그 부여 + 개명 API + 와이어)은 끝났고, **개명 UI(슬라이스 2)가 남았다.**

머지: Backend `0530ddd` · Client `cefa827`.
...
프로필 화면에서 이름을 바꾸는 자리. 백엔드 라우트는 살아 있는데 **클라에서 부를 데가 없다.**
가입 직후 이름이 `플레이어`라 전원이 같은 이름으로 시작한다 — 이걸 바꿀 수단이 슬라이스 2다.
