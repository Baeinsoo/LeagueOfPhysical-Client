...
## 🏹 Archery(활쏘기) 슬라이스 2 — 실측 통과 (2026-09-13)
...
두 클라로 실제 판을 돌려 확인했다(아래 "실측 체크리스트"). **아직 머지 전**이다.
...
- [ ] 먹힌 과녁이 양쪽에서 사라지는지 / 끊었다 붙였을 때 되살아나지 않는지 — **따로 재보지 않았다.**
      코드 경로는 리뷰에서 확인했지만 눈으로 확인한 항목은 아니다.
