...
## 🟠 매치 기록 통합 + 전적 목록 — 슬라이스 1 완료 (2026-08-21), 2·3 남음

**한 문장:** 판 하나를 `Match` **한 행**에 자기완결적으로 담아, 전적 조회가 5쿼리에서 1쿼리가 되게
하고 그 위에 LoL 전적 보기식 목록을 올린다.
...
| | 무엇 | 끝났다는 기준 |
|---|---|---|
| **2** | 전적 조회 라우트 (`GET /user/{id}/matches?limit=20`, lobby-server) | 1쿼리로 최근 20판을 내려준다 |
| **3** | 클라 전적 목록 (프로필 안 `ScrollView`, 판 카드) | 판 카드가 최신순으로 뜬다 |
