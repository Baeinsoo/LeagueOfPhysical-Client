...
## 🟠 시작 구간 클럭 정렬 — 걷기 시작 시 64cm 러버밴딩 (2026-08-11 규명, 미수정)
...
`rtt`가 304ms로 시작해 197ms로 내려가는 것과 관련 있어 **보이지만 확인된 바 없다.**
`ClockDilator` / `LeadController` 초기화 경로와 `SnapshotHistory` 앵커 조회를 봐야 한다.
**⚠️ 추론으로 단정하지 말 것** — 이 트랙에서 이미 세 번 틀렸다(`[[measure-before-theorizing-netcode]]`).
