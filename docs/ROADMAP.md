...
⚠️ **그리고 dev에서는 지금 아무 게임도 못 한다** — 아래 🔴 참고.
...
| | 항목 | 크기 | 유저가 겪나 |
|---|---|---|---|
| 🔴 | **dev·local 게임서버 이미지가 14커밋 낡았다** — `game-server:48cdce7`. 그 사이 와이어가 바뀌어(`ghost→stunned`, 매치 출발틱) 최신 클라가 붙으면 **6초 만에 서버가 끊는다**. dev에서 아무 게임도 못 한다 | 작음(배포) | 예 |
