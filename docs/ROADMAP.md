...
2. ~~**게임서버가 스스로 종료하기**~~ ✅ **완료(08-15)** — 아래 절.
3. **위치 정리기(active reconciler) — 조건이 생기면 착수** — 아래
   [유저 위치는 왜 lazy 보정으로 충분한가](#-유저-위치는-왜-lazy-보정으로-충분한가--액티브-정리기의-착수-조건)
   참조. **지금은 안 짓는다**(근거 있음). 착수 트리거 4개가 그 절에 있다.
