...
### 🟠 유저 위치(UserLocation) 전반 재정비 — 백엔드 + Unity 클라 (**클라 절반 완료, 2026-08-15**)

**왜 트랙으로 묶나:** 유저 위치는 사실상 **세션 상태**(이 사람이 지금 어디 있나)인데, 그 진실을
**아무도 소유하지 않고 여러 곳이 폴링해서 각자 해석**하고 있었다. 경합·재접속 루프·쓰기 부하가 전부
같은 뿌리에서 나온다.

> **진행 상황 (2026-08-15):** 클라의 "조회 주인 없음"은 닫혔다 —
> [유저 위치 조회를 서비스 하나로](#-유저-위치-조회를-서비스-하나로-2026-08-15-main-머지-f6a27f0) 참조.
> **남은 절반은 백엔드**이고, 클라에도 "해석 일원화"와 "push 수신"이 남아 있다. 아래 표가 현재 상태다.

#### 백엔드 쪽

| | 상태 |
|---|---|
| **정리 책임이 없다** — 매치가 끝나도 위치가 `GameRoom`으로 남는다 → 로비 진입 즉시 같은 게임에 자동 재접속. 룸 close→위치 정리 경로가 게임서버 `LOPRoom`의 `if (!Standalone)` 가드로 로컬에선 스킵되고 **프로덕션에도 잠재** | ⬜ **남음 — 이 트랙 1순위** |
| **만료가 없다** — 위치에 TTL이 없다. 클라가 죽으면 그 상태로 영원히 남는다. presence는 원래 TTL 있는 저장소가 표준인 영역 | ⬜ 남음 (티켓 쪽은 08-06 버려진 대기표 만료로 일부 해소) |
...
**이 트랙에 속하는 흩어진 항목들** (다른 절에 적혀 있던 것):
- **매치 생성 경로의 원자성** — 룸 생성·유저 위치 갱신·티켓 삭제가 HTTP를 건너 DB 트랜잭션으로 못 묶인다
- **"매칭 실패" 알림** — 지금은 아무 설명 없이 로비로 복귀해 **취소와 구분되지 않는다**
...
| **해석이 흩어져 있다** — 위치→다음 행동 `switch`가 상태마다 따로 | ⬜ **남음.** 서비스는 *값*만 주고 해석은 `CheckMatch`·`InMatchmaking`·`MatchLoadingViewModel` **3곳이 각자** 한다 |
...
1. **매치 종료 시 위치 정리** (백엔드 + 게임서버) — 사용자가 실제로 겪는 유일한 버그다. 매치가 끝나고
   로비로 가면 **같은 게임으로 도로 끌려간다.** 인게임 검증 ⑥을 못 돌린 자리이기도 하다.
   아래 파킹의 "매치 종료 시 유저 위치 백엔드 정리(Slice D 후속)"와 같은 건.
2. **위치 TTL** — 클라가 죽으면 그 사람은 영원히 그 상태다.
3. **클라 해석 일원화** — 지금 깨지는 건 없다. 위치 해석 *규칙*이 실제로 바뀔 때 같이 하는 게 값이 크다.
4. **`locationDetail` 타입 강화 / push** — 계약 변경이라 클·서 동시 작업.
...
| | |
|---|---|
| **"매칭 실패" 알림** | 지금은 아무 설명 없이 로비로 복귀한다(취소와 구분 안 됨). 위치 재정비 트랙 |
...
- **후속 후보 — `PhysicsFollower` 접기(공유 팩토리화)**: ⏸ **파킹(2026-07-20, 사용자 결정)** — 죽은 껍데기지만 동작 무해라 급하지 않음. 상세·재개 조건은 아래 파킹 표 참조.
