...
| 🟡 | **틱 루프가 멈춘 게 눈에 안 보인다** — 틱 루프가 코루틴이라 시스템 하나가 예외를 던지면 **루프 전체가 죽는다.** 콘솔에 NRE 한 줄이 남고 그 뒤로 아무 신호가 없어, 화면은 남의 새가 계속 움직이니 정상처럼 보인다(2026-09-06 라이브에서 실제로 그랬다 — 잡히는 순간 클라 시뮬이 통째로 멈췄는데 관전은 되고 있었다). **고칠 방향은 `try/catch`로 삼키는 게 아니다** — 결정론 시뮬에서 예외를 삼키고 계속 돌면 멈추는 대신 *틀린 상태로* 가고, 넷코드에선 그게 훨씬 찾기 어려운 desync가 된다. 멈추는 건 그대로 두고 **멈췄다는 사실을 드러내는 것**이 목표다(러너 상태 전이 + 눈에 띄는 로그, 필요하면 화면 표시). `RunnerBase`는 서버와 공유라 양쪽이 같이 바뀐다 | 작음 | 예 |
