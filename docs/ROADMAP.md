...
1. **프레임 예산 — 개발 빌드에서 측정.** 정상 구간에서 우리 시뮬은 프레임당 **0.3~1.0ms**인데
   프레임은 10~45ms를 오간다. 즉 **에디터에서 재는 긴 프레임은 대부분 에디터 오버헤드**이고,
   더 파봐야 게임이 아니라 에디터를 프로파일링하게 된다. `client-app-deploy`로 개발 빌드를 뽑아
   거기서 재야 의미가 있다. **프레임이 틱 간격(20ms) 안에 들면 이 이슈는 원리적으로 사라진다** —
   한 프레임에 밀리는 칸이 0~1개라 지난 칸 번호가 안 생긴다.
2. **시뮬 첫 틱 워밍업 (조건부).** 시작 첫 프레임의 `sim`이 **25.4ms**로 정상(0.5ms)의 50배다.
   *다만 상당 부분이 에디터 JIT일 수 있고 IL2CPP 빌드엔 없다* — ①에서 실측 후 판단.
   "미리 한 번 굴리기"를 곧이곧대로 하면 진짜 한 틱이 굴러 상태가 오염되므로 구현도 비자명하다.
