...
⚠️ **손바닥 치기는 코드만 올라갔고 배포가 안 됐다.** 6개 레포 푸시는 끝났지만
`gameserver-deploy`가 **GitHub Actions 장애**(2026-08-26 15:09~, Actions major outage)로 막혔다.
클러스터는 아직 `743f11a`(슬라이스 4)를 돌린다. **GitHub 복구 후 배포부터 다시 걸 것** —
그 전에는 클라가 보내는 새 메시지를 서버가 못 읽어 타격이 전부 무시된다.

**멀티터치는 PC에서 검증이 안 된다** — 에디터도 마우스도 손가락이 하나다. PC에선 마우스 회귀만 보고,
손가락 수·간격 효과는 **실기기 APK**로 봐야 한다.
...
| 🟡 | **판치기 손바닥 치기(멀티터치)** — 6레포 머지 완료, **배포는 GitHub 장애로 대기** |
...
| | 항목 | 크기 | 유저가 겪나 |
|---|---|---|---|
| 🔴 | **손바닥 치기 배포 마무리** — GitHub 복구 후 `gameserver-deploy` → PC 마우스 회귀 → 실기기 APK로 손가락 수·간격 확인 | 작음 | 예 |
...
### 🟡 판치기 손바닥 치기(멀티터치) — 2026-08-26, 6레포 머지 완료 / 배포 대기
