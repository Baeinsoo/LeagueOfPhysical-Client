...
## 🏹 Archery(활쏘기) 슬라이스 1 — 머지·배포 완료, 실측 미완 (2026-09-11)
...
1. **조준이 아래를 향한다** — 3인칭 카메라가 캐릭터를 내려다보는 각도를 조준각으로 그대로 썼다.
   화살이 머리 옆에서 나와 땅으로 꽂힌다. **위로 겨눌 수 있는 한계도 20도뿐**(최종 리뷰 M3).
   고칠 곳은 코드가 아니라 **Archery 씬의 `CameraController` 상하 한계·시작 각도**.
2. **실측 재개** — 남의 화살이 보이나 / 내 화살이 두 번 그려지지 않나(자기 발사 필터 검증).
3. **판 종료가 한쪽만 된다** — MPPM 클론이 방에 남는다. 옛 방으로 계속 입장하려던 것과 같은 뿌리로 의심.
4. **`[RemoteProbe]` 임시 진단 로그 제거** (`RemoteInputSystem.cs`, 현재 커밋 안 된 채 워킹트리에 있음).
5. 미머지 브랜치: 클라 `feature/archery-visible-arrow`(화살을 크고 빨갛게 — 실측 편의).
