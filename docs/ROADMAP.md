...
### 🟠 캐시 계층(`CacheCrudRepository` + `DaoRedisBase`) 정리 — 백엔드 공용

B 슬라이스(로비 자가치유) 최종 리뷰가 짚은 둘. **이번 브랜치가 만든 게 아니라 공용 패키지의 기존
동작**이고 고치면 세 앱 전부에 영향이 가서 별건으로 뺐다(2026-08-04, 사용자 결정).

| | 항목 | 왜 |
|---|---|---|
| 🟠 | **`CacheCrudRepository.save()`가 캐시를 DB 쓰기 *전에* 지운다** | 지우고 쓰는 사이에 들어온 조회가 **옛 값을 캐시에 되살린다.** 같은 함정을 이미 두 번 겪어(티켓 취소, 위치 조건부 쓰기) 둘 다 "DB 먼저, 캐시 나중"으로 짰는데, 공용 `save()`만 반대로 남아 한 파일 건너 서로 어긋나 있다 |
| 🟠 | **Redis TTL이 슬라이딩이다** | `DaoRedisBase.findById`가 읽을 때마다 `expire`를 다시 건다. 클라가 1초마다 폴링하므로 **그 키는 영원히 만료되지 않는다** — "TTL이 결국 정리해 준다"를 안전망으로 쓸 수 없다 |

**왜 지금 아픈가:** B 이전엔 조회가 끝날 때마다 `save()`가 캐시를 지워서 낡은 항목이 한 폴링 이상
살아남지 못했다(대신 그 쓰기가 lost-update를 만들었다 — 그게 B가 고친 것). 이제 읽기가 쓰지 않으므로
**낡은 캐시 항목이 무기한 남을 수 있다.** 위험 창은 좁지만(Redis 왕복 한 번) 결과는 오래간다:
캐시에 `None`이 박히면 자가치유가 "멀쩡하다"고 판단해 아무것도 안 고치고, 그 유저는 방이 닫힐
때까지 로비에 남는다. **치유 대상이 되는 낡은 값은 스스로 고쳐지지만(치유 경로가 캐시를 지운다),
"멀쩡하다고 판단된" 낡은 값만 고쳐 줄 주체가 없다.**
...
| | 항목 | 왜 |
|---|---|---|
| 🟠 | **로비 자가치유의 lost-update** | 위치 조회가 조건부 쓰기가 아니라 read-modify-write라, 티켓 조회(HTTP 1왕복) 사이에 Director가 `GameRoom`을 쓰면 옛 `WaitingRoom`으로 덮을 수 있다. 창이 수백 ms → 수 ms로 줄었을 뿐 0은 아니다. 근본 해법(조건부 쓰기/CAS)은 lobby-server 전체에 걸리는 별도 작업 |
