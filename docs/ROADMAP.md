...
②는 낌 자리 0개(구역 0개 — 낌점 후보 748개, 지형에 닿는 자리 4781개 중 낀 곳 없음).

**①은 정직하게 미해결이다.** 네 스폰(`PlayerSpawn_1`~`4`) 전부 탐색은 경로를 찾았지만
(191/191/190/187회 날갯짓), 넷 다 진짜 커널 재생에서 어긋났다. 더 정확한 원인: 0.1m 눈금은
낙하 초반 한 틱치 하강(약 0.028m)보다 굵다 — 그래서 자유낙하 첫 몇 틱은 눈금 모델 안에서
**아예 안 내려간다.** 이 편향이 틱마다 새 쪽으로 쌓이고, 다음 틱은 그렇게 부풀려진 높이에서
또 쌓인다 — 187~191번의 날갯짓에 걸쳐 누적하면 허공에 뜬 높이가 약 7m가 된다. 여기에 두
원인이 더 얹힌다: 되짚기가 살아남은 상태 중 **가장 낮은(=마진이 가장 얇은) 경로**를 고르고,
이동 커널은 스침도 히트로 세는 skin width로 sweep하는데 탐색의 자유공간 판정은 정적 겹침
검사라 **둘이 같은 질문을 하지 않는다.** 탐색이 찾은 경로는 진짜 물리에서 버티지 못한다.

**이건 설계가 실패한 게 아니라 의도대로 작동한 것이다.** 스펙은 정확히 이 상황을 예상해 "찾은
경로를 진짜 커널로 재생해 증명"하는 단계를 넣어 뒀다 — 그 단계가 없었다면 도구는 자신 있게
✅만 찍었을 것이고 우리는 틀린 답을 믿었을 것이다. 도구는 자기 결과를 스스로 증명하지 못했다고
정직하게 보고했다.

**준비해 둔 두 처방은 이 크기에 안 맞는다.** 목표 높이를 바꿔도 소용없다 — 편향은 경로 하나가
아니라 틱 하나마다 붙기 때문이다. 눈금을 좁혀도 단순 비례가 아니다 — 날갯짓 한 번(0.46m)을
0.05m 눈금으로 나누면 반대 방향으로 나머지가 남아, "더 나은 값"이 아니라 예측 불가능한 값이
된다.

**다음 슬라이스는 고치기가 아니라 접근을 바꾼다.** 전수 탐색이 정확한 높이를 들고 다니게
만드는 대신 **봇 한 마리를 진짜 커널 위로 날린다**: 앞을 진짜 물리로 스캔해 세로로 뚫린 틈을
찾고, 그 틈을 겨냥해 날갯짓한다. 궤적을 하나만 시뮬레이션하므로 상태를 묶어 셀 필요가 없고,
그래서 반올림도 표류도 아예 생기지 않는다. 비용도 준다 — 자리 하나당 상태 반복 약 2억 번 대신
한 번의 실행(약 2,880틱)이면 된다.

두 방식은 서로의 사각을 메운다: 봇이 통과하면 무충돌 경로가 **있다**는 걸 증명하고, 전수
탐색은 맵이 **불가능**하다는 걸 보일 수 있는 유일한 방법이다. 각자 상대가 못 하는 일을 한다.

**봇은 한 번이 아니라 자리마다 한 번씩, 맵의 스폰 마커 수만큼 날려야 한다** — 상수로 박지
않는다(이 맵은 4자리, 8인 맵이면 8자리). 이유가 바로 ①이 있는 이유다: 자리마다 높이가 다르고
(이 맵은 y = −6, −1, 4, 9 — 15m 차이), 어느 한 자리만 클린런이 안 된다면 그건 자리 배정에
박힌 불리함이다. 자리 하나만 날려 보고 맵이 괜찮다고 하면 ①이 찾으려던 바로 그것을 지워버린다.

`FlappyRaceSlice/Logic/FlappyGapAiming`(EditMode 테스트 16개)은 이미 옮겨져 있고, 처음부터
"막힘/뚫림" 표를 **부르는 쪽이 진짜 물리로 재서** 넘기도록 짜여 있다 — 이 슬라이스의 일부는
이미 있다. 그리고 이건 아래 "오토파일럿이 스턴 예산을 못 지킨다" 항목과 **같은 기계**다 —
한 번 지으면 둘 다 해결된다.

**실행이 남긴 후속 과제 둘.** ⓐ 재생이 어긋나도 보고서는 "어긋났다"고만 찍는다 — 몇 번째 틱,
어느 위치에서 갈렸는지가 없다. 사람이 원인을 진단하려면 그게 제일 먼저 필요하다.

ⓑ 지금 ③(스턴 예산)은 `spawns[0]` 하나만 놓고 한 번 계산한다. 이 맵에선 무해하다 — 네 자리가
전부 x = −2로 같아서 예산도 같다. 하지만 스폰이 x축으로 어긋난 맵이 오면 자리마다 예산이
달라지는데, 보고서는 그중 한 자리의 숫자를 마치 전원 것처럼 찍는다. 이 전제는 코드 어디에도
적혀 있지 않다. 알려진 한계로 남긴다 — 트리거는 "스폰들의 x가 서로 다른 맵". 다음 슬라이스가
③도 자리별로 계산하거나, 최소한 모든 스폰이 같은 x인지 확인해 다르면 경고를 찍어야 한다.
