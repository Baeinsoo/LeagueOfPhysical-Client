...
- **첫 틱 스파이크 (시계 아님).** 수정 후에도 `[ReconSpike]`가 매치 첫 틱에 1건 남는다 —
  `err` `0.040~0.080`(걷기 4m/s 기준 1틱 분량), `snapAge`는 정상(12), `dAvg`도 정상(-2.8). 같은
  회차에 `pruneTot=3`/`seqGapTot=3`이 처음 관측됐다(이전엔 0). "시작 직후 입력 파이프라인이 막
  가동돼 처음 몇 개가 늦거나 버려진다"는 설명이 그럴듯하지만 **로그로 확인한 바 없다** —
  추정으로 적어둔다. 시계 지표는 전부 정상이라 이 트랙과는 별개다.
