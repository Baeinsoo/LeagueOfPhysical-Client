...
| | 항목 | 왜 |
|---|---|---|
| 🟠 | **배럴에 부수효과가 생겼다** | `@lop/server-core`에서 타입 하나만 import 해도 `mkdirSync` + winston transport 2개 + `createClient` + lua 4파일 읽기가 실행된다. 서비스 진입점 4개 기준 *신규* 부수효과는 0이라(앱들이 원래 다 로드했다) 오늘 고장은 아니지만, 테스트 경로가 이것 때문에 죽어 `jest.setup.js`로 봉합했다. 정식 해소는 서브패스 export 또는 지연 초기화 |
| 🟡 | `packages/server-core`에 `lua/`가 없다 | `redis.loader.ts`가 CWD 상대로 읽는다. 지금은 그 CWD로 배럴을 로드하는 프로세스가 없어 무해하나, **이 패키지에 테스트를 추가하는 순간 ENOENT** |
...
| 🟠 | **티켓 발급이 유저당 유일하지 않다** | 요청 검증이 위치를 읽는 시점과 티켓을 만드는 시점이 원자적이지 않아 중복 요청이 티켓 두 장을 만들 수 있다. 지금은 확정 트랜잭션이 참가자의 *다른* 티켓까지 함께 소비해 막지만, 근본은 발급 단계의 유일성 제약 |
