...
> **다음 (아직 안 함) — 층 가르기.** z=62에 배경 블록 64개가 **이미 있다**. 게임 평면은
> 선명·고대비·밝은 테두리로, 배경은 채도↓·안개·아웃포커스로 갈라 "선명하면 닿는 것"을
> 눈이 배우게 한다. 투영 방식과 무관하게 거의 모든 2.5D가 함께 쓰는 조각이다.
> spec `2026-09-15-flappy-map-25d-depth-alignment-design.md` §3②.
