...
> **✅ 입력 폐기는 재현되지 않았다 (2026-08-11).** Reset을 누르지 않고 매치 시작부터 25초를 덮어
> 잰 회차에서 **`pruneTot=0` / `seqGapTot=0`**. `prune=162`는 **같은 날 고친 서버 틱 시드 버그**
> (`behind by 1680`) 상태에서 나온 관측이었을 가능성이 크다. 입력 파이프라인은 시작 구간에서도
> 한 틱도 안 버린다.
>
> **🟠 다만 시작 구간이 깨끗하지는 않다 — 서명이 다르다.** 같은 회차에서:
>
> | | 시작 포함 | 안정 구간 |
> |---|---|---|
> | `corrections` (하드 롤백) | **33** | 0 |
> | `reconMax` | **0.080 (8cm)** | 0.004 (4mm) |
> | `reconAvg` / `reconLast` | 0.000 | 0.004 |
>
> 롤백 문턱(0.06)을 넘겨 실제로 되감겼고, **이후엔 완전히 가라앉는다**(avg·last가 0). 즉
> *"시작 직후 조작이 안 먹는"* 증상의 원인 후보는 **입력 폐기가 아니라 롤백**이다 — 별개 문제이므로
> 별도로 다룬다. **⚠️ 1회차뿐** — 33회·8cm가 재현되는지, 어느 구간에 몰리는지 미확인.
>
> **재는 법:** 입장 후 **Reset을 누르지 말고** 15~20초 뒤 Dump. 측정은 위 4항목과 같은 이유로
> **클론에서** 한다.
