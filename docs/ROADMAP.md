...
| | 항목 | 크기 | 유저가 겪나 |
|---|---|---|---|
| 🟢 | **판치기 슬라이스 3 실플레이 검증** — 코드·배포는 끝났고 두 클라로 눈 검증만 남음(화면 필요) | 작음 | 예 |
...
### 🟡 판치기 슬라이스 3 — 타격 입력 + 힘 커널 (2026-08-25, 7레포 머지 + 게임서버 배포 완료 / 실플레이 검증 남음)
...
**검증 상태**: 서버 EditMode 547/547 · 클라 컴파일 클린(콘솔 에러 0) · 게임서버 CI 빌드 성공(IL2CPP 2아치) ·
ArgoCD 롤아웃 확인(room-server가 새 ConfigMap `270bc5b`를 물음). **눈으로 보는 검증은 아직** —
판치기는 `min_players: 2`라 MPPM 클론이 필요하고, MPPM은 창이 그려져야 어셈블리가 뜬다(포커스 필요).

머지: GF `18bfe9e` / Shared `3e9aa2a` / Client `91e12d3` / Server `270bc5b` /
MD-Client `4bbdf6b` / MD-Server `7a477ba` / infra `b54a073`.
