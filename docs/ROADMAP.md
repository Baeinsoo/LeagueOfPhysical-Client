...
## 🏹 Archery(활쏘기) 슬라이스 1 — 완료·실측 통과 (2026-09-11)

새 게임 모드. **제자리에 서서 왼손 시점 / 오른손 당겼다 놓기**, 화살은 포물선. 과녁·점수·함정은
슬라이스 2~3 몫으로 **의도적으로 없다**. 8레포 머지·푸시·배포 완료(EditMode 1406/1406).
두 클라 실측 통과 — 겨눈 대로 날아가고, 남의 화살이 보이고, 판이 끝나면 양쪽 다 로비로 나온다.
...
화면 한가운데가 곧 화살이 갈 곳이어야 하므로, 카메라를 **조준선 위**에 세운다 —
도는 중심을 눈높이로 올리고(`pivotHeight`) 그 중심보다 **0.4m 앞**에 선다(거리 음수).
...
- `CenterStage` 콜라이더가 캡슐(r=2,h=0.4)이라 사실상 구 — 과녁이 무대 위에 뜨는 슬라이스 2 전에 정리.
