...
| 🟢 | **Flappy에 잘 난 것에 대한 보상이 없다** — 전진이 상수라 실력이 반영되는 통로가 **충돌 8.8m · 스침 수 cm** 둘뿐이고 **둘 다 실수 감점**이다. 완벽하게 나는 둘은 영원히 동점(라이브에서 공동 1위로 확인). 옛 싱글 프로토타입의 `FlappyBoostZone`(통과 시 1.8배 · 1.6초)이 "좁은 틈을 정확히 뚫으면 빨라진다"는 보상 구조라 시뮬로 올리는 것이 후보. **결승선 연출 판단도 여기에 걸려 있다** — 등수가 실제로 갈리기 시작해야 가릴 것이 생긴다 | 중간 | 예 |
