...
> **남은 것 = 해석 일원화 · push · `locationDetail` 타입 강화** (셋 다 "지금 깨지는 건 없다").
...
| | 항목 | 실제로 벌어지는 장면 |
|---|---|---|
| 🟠 | **"매칭 실패" 알림** | 대기 화면이 설명 없이 닫히고 로비로 온다. 유저는 **내가 취소한 건지 사람이 안 모여 실패한 건지 알 수 없다** — 출석부에 위치만 있고 *왜 바뀌었는지*가 없어서 화면이 둘을 구분할 근거가 없다 |
| 🟠 | **위치 일괄 정리가 무조건 쓰기** | 매치 종료 시 참가자 전원 위치를 조건 없이 `None`으로 덮는다. 종료 직후 **곧바로 재매칭을 건 사람은 그 쓰기에 덮여 대기 중 로비로 튕긴다**. 고치려면 "아직 그 방에 있는 사람만" 같은 **조건부 갱신 API를 백엔드에 신설**해야 해서 08-15에 미뤘다(기존 문제) |
| 🟡 | **매치 생성 경로의 원자성** | 룸 생성·위치 갱신·티켓 삭제가 **서로 다른 서버**라 HTTP를 건너 한 트랜잭션으로 못 묶인다. 중간에 실패하면 어중간한 상태(방은 있는데 위치는 `Matchmaking`)로 남고, 그 사람은 매칭도 게임도 못 한다. 실제 발생 관측은 아직 없음 |
| ⬜ | **클라 해석 일원화** | 조회는 `UserLocationService` 한 곳으로 모였지만(08-14), *그래서 뭘 할까*의 판단은 `CheckMatch`·`InMatchmaking`·`MatchLoadingViewModel` **3곳이 각자** 한다. 지금 셋이 어긋나 있진 않다 → **해석 규칙이 실제로 바뀔 때 같이 하는 게 값이 크다** |
| ⬜ | **`locationDetail` 타입 강화** | 위치에 딸린 메모가 자유형식 JSON이라 "매칭 중이면 티켓 id가 들어있다"가 **코드 규약일 뿐**이고 클·서가 각자 정의한다. 계약 변경이라 클·서 동시 작업 |
...
**미룬 것(각각 근거 있음):** 위치 일괄 정리가 무조건 쓰기라 새 위치를 덮을 수 있음(조건부 갱신 API 신설
필요, 기존 문제) · 종단 상태가 편도문이라 느린 콜드 스타트 룸이 복구 불가(콜드 pull 첫 매치 실패는
