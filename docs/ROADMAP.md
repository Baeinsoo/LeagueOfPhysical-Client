...
| 🟢 | **내 엔티티 id 두 개는 개념이 다르다 — 섞어 쓰는 곳을 없애야 한다** — `gameDataStore.userEntityId`는 **정체**(이 판에서 서버가 나에게 배정한 id, `GameInfoToC`를 받는 순간 채워지고 내가 잡혀 몸이 사라져도 그대로 남는다), `playerContext.entityId`는 **지금 살아 있는 내 몸의 손잡이**(`EntityBinder`가 내 엔티티 소멸 때 `.actor`와 함께 내린다)다. 같은 값을 담지만 **수명이 다르다** — 스폰 *전부터* 유효한 쪽이 앞의 것, 몸이 *있는 동안만* 유효한 쪽이 뒤의 것. 그래서 `playerContext.entityId != null`이면 **그 엔티티가 레지스트리에 있다**는 불변식이 성립한다(클라의 레지스트리 제거는 `EntitySpawner.FlushDespawns` 한 곳뿐이고 같은 호출로 `EntityDestroyed`를 쏜다). **할 일은 합치는 게 아니라 가려 쓰는 것**이다 — 지금 스폰 시점 코드가 앞의 것을, 나머지가 뒤의 것을 쓰는 건 설계가 아니라 *우연*이다. 정체를 물으면서 뒤의 것을 읽는 자리를 찾아 고친다(이번에 `RaceFinishViewModel`이 그랬다 — 판이 끝나며 엔티티가 정리되면 화면의 등수가 사라졌다). 그리고 방어적 null 재확인이 붙은 다섯 곳은 불변식이 생겼으니 걷어낼 수 있다. 두 인터페이스에 그 뜻을 주석으로 박아 뒀다 | 중간 | 아니오 |
