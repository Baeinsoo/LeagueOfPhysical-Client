...
- **다음 = S5** — 매니저/팩토리/스포너 구조 재작업(거대 Creator → 표준 스포너/바인더 분해) + `entityMap`↔`EntityRegistry` 이중화 축소 + **얇은 `IEntity` 완전 삭제**(제네릭 재타입 — 서버 매니저 `GetAllEntitySnaps`가 파사드를 IEntity로 읽어 매니저 재작업과 묶여 있어 S5로 미뤄둠). (기회 정리: `LOPRunner.cs` `LOPEntities` 지역변수 rename.)
