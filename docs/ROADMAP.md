...
### ⏭ 다음 빌드 테스트에서 확인할 것 (미검증 항목, 08-09 유예)

**환경:** 서버를 **스탠드얼론 빌드**로 띄운다(에디터 오버헤드·잡음 바닥 제거). 클라도 빌드면 더 좋다.

| 볼 것 | 통과 기준 | 왜 로컬에선 못 봤나 |
|---|---|---|
| **lead 평형점** — `maxD` 분포 | 정상 구간에서 `maxD ∈ [-3,-1]`에 눌러앉는다 | 잡음 5틱 > 신호 1~3틱 |
| **prune / seqGap** | 정상 구간에서 0 유지 | 9초 멈춤이 누적 수치를 오염 |
| **마진 수렴** | `AheadMargin`이 천장(100ms)에 붙어 있지 않다 | 로컬에선 계속 천장이었다 |
| **클라 시작 멈춤** | 멈춤 ≤ 1초면 대책 불필요(50틱 = 7프레임에 소화) | 메인 에디터만 겪는 증상 |

> 수치는 **`[HudDump]`의 `maxD` / `prune` / `seqGap` / `stalls` / `behindMax`** 로 본다. 단
> **누적값은 멈춤이 삼키므로**, 멈춤(`stalls>0`)이 낀 회차는 버리고 다시 잰다.

**같이 정리할 것:** `LeadController`의 나머지 경계값(`maxMargin` 0.1s, step 10/2ms, `DefaultMargin`)이
아직 하드코딩이다. 바닥값만 틱에서 유도했으니, 검증하는 김에 나머지도 틱 배수로 맞춘다(동작 무변경).
