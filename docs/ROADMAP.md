...
- **Tier-2/3 (별도 슬라이스):** #7 `WorldEventBatch` 단일 envelope 미구현(개념별 패킷 `DamageEventToC` 등) · #8 static `EventBus.Default`(DI/R3 밖) · 크리/회피 상수 하드코딩 · `ctx.Target` 항상 자기자신(실 타게팅 없음) · 링버퍼 3중복.
