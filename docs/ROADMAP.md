...
- **`CenterStage` 콜라이더 정리** — 캡슐이 스케일에 먹혀 사실상 지름 4m 구인데 보이는 것은 반지름
  2m·높이 0.4m 원기둥(슬라이스 1부터 이월). 이번에도 못 했다 — **Art 서브모듈이 원격보다
  4커밋 뒤처져 있고**, 사용자의 **커밋 안 된 재질 변경 6건 + 미추적 에셋팩**이 얹혀 있어 건드리면
  그 상태를 흔든다. 과녁은 무대 위 공중에 뜨고 적중은 우리 선분-구 산수가 하므로 이 슬라이스에
  콜라이더가 쓰이지 않아(load-bearing 아님) 미뤄도 안전했다.
- `#ArcheryConfig`의 `min_separation = 1.2`가 가장 큰 과녁 둘의 반지름 합과 **정확히 같아서** 둘이
  접한 채 뜰 수 있다. 서버 테스트 픽스처는 1.4를 쓰는데 배포 데이터만 1.2 — 맞출 것.
- `ArcheryStateBroadcastSystem`(재접속 대응)에 테스트가 없다. 이 슬라이스가 사건 → 상태로 갈아엎은
  바로 그 이유를 재는 코드가 없는 셈(선례 `PanchigiTurnSystem.BroadcastIfChanged`도 같은 공백).
- 결과 화면에 점수 숫자가 안 보인다(등수만). 슬라이스 3 재미 판정 때 있으면 낫다.
