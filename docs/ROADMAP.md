...
## 📍 지금 어디 (2026-08-27 세션 끝)

**판을 눈으로 읽을 수 있게 됐다.** 동전이 앞뒤로 다르게 생겼고(안 뒤집힘 = 남색 민무늬,
뒤집힘 = 미소녀 코인), 화면 위에 `뒤집힘 2 / 4`가 뜬다. 승리 조건이 원래 "전부 뒤집힘"이라
그대로 **"미소녀가 전부 보이면 이김"** 이 된다 — 서버 로직은 한 줄도 안 바뀌었다.

같이 나온 것: 판치기가 **main에서 아예 깨져 있었다**(방에 들어가는 순간 DI 예외 → 로비로 튕김).
원격 외삽 뷰가 `EntityBinder`에 새 의존을 추가하면서 판치기 스코프만 등록이 빠진 것이다.

**판치기가 이제 책 위에서 벌어진다.** 판을 10×10m에서 **2.8×4m 책 한 권**으로 줄였다 — 크기는
지어낸 게 아니라 실제 100원(지름 24mm)과 교과서 판형에서 끌어냈다. 동전이 책 긴 방향으로 서고,
세게 치면 실제로 **판 밖으로 나가며(낙), 그러면 판이 통째로 처음 세팅으로 돌아간다.**
낙은 이제 **공짜가 아니다** — 떨어뜨린 사람 앞으로 쌓이고, 설정한 횟수(기본 3)에 닿으면 그 사람은
판에서 빠진다. 둘이 하면 상대가 그대로 이긴다. 화면에 `낙 1 / 3`이 뜬다.
판이 좁아지자 그동안 도달 불가라 숨어 있던 것들(판 끝 동전을 짚으면 치기가 거절되던 레이어 버그,
판 윗면과 맵 지면이 같은 높이라 나던 z-파이팅)이 드러나 같이 고쳤다.

배포도 밀렸던 것을 따라잡았다 — 게임서버 이미지가 와이어 변경보다 뒤처져 **dev에서 아무 게임도 못 하던**
상태였고, 지금은 dev에서 판치기 한 판을 끝까지 돌려 확인했다.
...
| | 항목 | 크기 | 유저가 겪나 |
|---|---|---|---|
| 🟢 | **환경을 바꾸면 다른 환경 계정이 조용히 지워진다** — 자격증명 키가 `LOP.Auth.{프로필}.Credential`이라 환경이 안 들어간다. dev↔local을 오가면 한 슬롯을 덮어써 앞 환경 계정이 유실된다(2026-08-27 실제로 잃음). 키에 환경을 넣으면 끝. **재시작 없이 환경만 바꾸면 401 자동복구도 안 걸린다**(복구는 부팅 때만) | 작음 | 아니오(개발자) |
| 🟢 | **접촉 반복 해석기(iterative contact solve)** — 새끼리 겹쳐 얹히면 영원히 떤다(2026-08-26 2인 실측: 서버가 2주기 한계진동). 접촉을 틱당 한 번 모으고 속도 제약을 여러 번 반복 — Box2D/PhysX 표준. `grounded` 플래그로 때우지 않기로 함(맵만 보므로 3단 쌓임에서 깨지고, 남의 새 것은 와이어로 늦게 온다). 상세·숫자는 `specs/2026-08-26-flappy-solid-map-design.md` §9 | 중간 | 예 |
