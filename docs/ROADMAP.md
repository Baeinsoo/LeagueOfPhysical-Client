...
### 다음에 값어치가 큰 것
0. **"운이냐 실력이냐"의 재판정 (09-22)** — 이제 **한 자리에서 3발**을 같은 과녁에 꽂고
   오른쪽 위 기록판에 **점 3개가 군집**으로 남는다. 볼 것은 하나다 — *점이 한 덩어리로
   모였는데 중심에서 벗어났는가*(→ 리드를 그만큼 더/덜 주면 되는 **고칠 수 있는 상태**)
   *아니면 흩어지는가*(→ 조준이 안 잡히는 것이니 다른 축).
   ⭐ **점수 띠는 09-22에 정식 10링(10~1점)으로 이미 늘렸다** — 이제 기록판의 군집과 점수가
   같은 해상도로 말하므로, 그 둘을 나란히 놓고 판정하면 된다.
