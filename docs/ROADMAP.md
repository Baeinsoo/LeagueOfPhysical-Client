...
> ✅ **닫힘 (2026-08-11)** — 빌드가 아니라 **MPPM 클론**에서 재서 확인했다. 두 회차 모두 `stalls=0`
> (이전 메인 에디터는 `stalls=1`, `behindMax=463`). "메인 에디터만 겪는 증상"이 수치로 확정됐고,
> 넷코드 대책은 필요 없다. 상세는 위 "미검증 4항목 전부 통과".
...
> **🟠 아직 열려 있다 (2026-08-11 확인).** 같은 날 클론에서 잰 두 회차는 **이 항목을 덮지 않는다** —
> `InputTimingStats.Reset()`이 누적 카운터까지 지우므로 `pruneTot=0`은 *Reset 이후*만 뜻하고, 문제
> 구간은 그 앞에 있다. **재는 법: 입장 후 Reset을 누르지 말고** 15~20초 뒤 Dump해서 `pruneTot` /
> `seqGapTot`을 본다(162 근처면 재현, 0이면 옛 관측이 로컬 아티팩트였다는 뜻). 측정은 위 4항목과
> 같은 이유로 **클론에서** 한다.
