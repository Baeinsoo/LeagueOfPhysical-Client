...
**남은 것**: PlayMode 테스트 공백(§아래). (접촉 반복 해석기는 몸싸움을 안 쓰게 되면서 동기가 사라졌다 — 아래 참고.)
...
| ⏸ | ~~**접촉 반복 해석기** — 새끼리 겹쳐 얹히면 영원히 떤다~~ → **동기 소멸(09-06 확인).** 몸싸움 자체를 안 쓴다 — 남의 새를 예측 대신 **스냅샷 보간**으로 그리기로 하면서 `BodyCollisionSystem`·`BodyOverlap`·`VerticalBounce` 호출처가 0이 됐다(클래스는 남아 있고 되살릴 조건은 추격자 설계 §3①에 적혀 있다). **떨림을 고친 게 아니라 떨림이 나는 코드를 안 돌리게 된 것**이므로, 몸싸움을 되살리면 이 일감도 같이 돌아온다 | 중간 | 예 |
...
| 🟢 | **낙하 카메라 — 남은 절반** | §11 | ① 아래 보기 = 09-02가 피치 −20~80 → **−90~90**으로 열어 해소 / ② 거리 상한 `maxDistance = 20f` **그대로**(다이브는 초속 40m) / ③ 속도가 화면에 **안 나타남** — 대기 슬라이스가 *"근경이 비어 속도감이 없다, 평평하고 먼 판은 그 일을 못 한다"*고 스스로 남겼다. 남은 것은 **자세별 카메라**(다이브=뒤로+FOV)와 근경 표현 |
