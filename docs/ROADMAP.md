...
S5는 **S5a / S5b + 어휘 rename**으로 분할됐다. **S5a(IEntity 삭제 + 모션 접근자 통일) 완료·머지**(아래 Done 원장).

- **다음 = S5b** — 거대 `CharacterCreator`(데이터 조립 ↔ Unity 뷰 조립 혼재)를 표준 스포너/바인더로 분해 + `entityMap`(뷰-인덱스)를 매니저에서 뷰-스포너로 재배치 검토 + "매니저 규격(`IEntityManager`) 존재 재고". S5a에서 미룬 깊은 매니저/스포너 재작업.
- **어휘 rename 패스**(별도 기계적 커밋): `entity`/`entities`/`LOPEntities` 식별자 → `actor`. S5a는 의미 변경만 했고 rename은 리뷰 가독 위해 분리(S5b와 함께 or 단독).
