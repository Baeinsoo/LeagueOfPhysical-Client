...
## 🏹 Archery(활쏘기) 슬라이스 2 — 코드 완료, 실측 대기 (2026-09-11)
...
⚠️ **실측은 아직 안 했다.** 6레포 18커밋 코드는 다 됐고 EditMode 테스트도 통과했지만, 두 클라를
띄워 눈으로 확인하는 절차(아래 "실측 체크리스트")는 이 문서를 쓰는 시점까지 돌지 않았다.
...
### 실측 체크리스트 (사람이 두 클라로 — 아직 안 함)

- [ ] 양쪽이 **같은 과녁**을 보나(위치·크기). 이게 깨지면 나머지는 볼 필요가 없다.
- [ ] 맞히면 점수가 오르나 — **AutoMapper가 실제로 점수를 실어 오는지**가 핵심(정적 근거는
      강하지만 이 프로젝트에 "매핑 누락이 에러 없이 조용히 멈춘" 전례가 있다).
- [ ] 먹힌 과녁이 **양쪽 화면에서** 사라지나.
- [ ] **한쪽을 끊었다 다시 붙였을 때** 이미 먹힌 과녁이 되살아나 보이지 않나.
- [ ] 60초 뒤 순위가 서버 로그 `[Outcome]`과 맞나.
- [ ] 양쪽 다 로비로 나오나(슬라이스 1 항목의 회귀).
