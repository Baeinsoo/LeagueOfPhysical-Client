...
| | |
|---|---|
| **원인 자체를 줄이기** | 입력 미스 시 마지막 입력 반복(`netcode-redesign.md` Phase 3 옵션 A) · `LeadController.minMargin`(현재 **0**) 하한을 복구본 지연(1틱) 이상으로. **다음 슬라이스** |
| 렌더 `minCorrection` | 지금은 불필요. 작은 보정이 화면에 보이면 그때 |
| 실환경 재측정 | 이 판단은 **로컬 조건 기준**이다. 원격·다기기에선 미세 오차가 상시 존재해 재생 빈도가 달라질 수 있다 |
