...
## 📍 지금 어디 (2026-08-25 세션 끝)

**판치기를 실제로 칠 수 있다.** 판을 끌어 치면 동전이 튀고 뒤집힌다 — 슬라이스 3(타격 입력 + 힘 커널)을
8레포에 머지하고 두 클라로 끝-끝 확인했다. 힘은 서버가 정하고, 세기는 *동전이 판에 닿은 정도*(덮임)로
계산한다 — 포개진 동전은 직접 힘을 안 받고, 반쯤 걸친 동전은 절반만 받는다.

이 과정에서 **에셋 배달 경로**를 몰라 세 번 헛짚었다. 이 프로젝트는 모바일·서버가 S3에서 어드레서블을
받는 게 기본이고, **구운 결과물을 올려야(`content-deploy`) 반영된다** — 서버 레포만 배포해서는 새 에셋이
안 보인다. 자세한 경위는 아래 슬라이스 3 원장에 남겼다.
...
| | 항목 | 크기 | 유저가 겪나 |
|---|---|---|---|
| 🟢 | **판치기 슬라이스 4 — 턴 상태 기계 + 낙(落) 판정 + 전용 맵** | 중간 | 예 |
