...
### 다음에 값어치가 큰 것
0. **새 조작의 실물 판정 (09-19)** — 관성을 버린 조준 + 되살린 떨림(0.3°) + 세운 과녁이
   *조준 실력*으로 읽히는가. 아니면 다음 수는 **조준선을 걷어 낙차 읽기를 되살리는 것**이다
   (지금은 착탄점을 그려 주므로 거리별 홀드오버 0.8°~6.0°를 스스로 익힐 일이 없다. 젤다에도 그 선은 없다).
