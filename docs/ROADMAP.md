...
**S1~S5 전부 완료·머지 — 엔티티 Unity 레이어 재구조화 트랙 종결.** S5a(IEntity 삭제 + 모션 접근자 통일) + S5b(Creator 데이터/뷰 분리 + entity→actor rename) 완료(아래 Done 원장). 엔티티 = 순수 C# `World.Entity`(데이터+시뮬), Unity = 얇은 프레젠테이션(Creator=데이터 직원 / 반응형 뷰 스포너=뷰 직원), 문자열/구조 배선·병렬 컴포넌트 시스템·파사드 소멸.

- **의도적 보류(별도 슬라이스, 필요 시)**: `entityMap`→스포너 이전 / `IEntityManager` 규격 해체(YAGNI — 매니저 현행 유지) · **PhysicsBody 순수 포트화(Y)** — World.Entity의 Unity 핸들을 포트로 정화(`[[physicsbody-port-purity-deferred]]`, 코어는 이미 IMotionBridge로 격리).
- **비차단 후속(기회 정리)**: register→PhysicsBody 순서 불변 주석(동기 발행 의존 명시) · S5b Task1 rationale 주석 2개 복원 · 서버 `CharacterCreationDataCreator` param `lopEntity`→`actor`.
