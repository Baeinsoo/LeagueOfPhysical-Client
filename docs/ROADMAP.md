...
## 🟠 매치 결과 + 레이팅 — A·B·C·D1 완료 (2026-08-21), D2(프로필)만 남음

**한 문장:** 한 판이 끝나면 등수를 남기고 그 결과로 실력 점수를 갱신해, 다음 매칭이 그 점수로 사람을
붙이는 고리를 닫는다. **고리가 닫혔고(C) 유저에게 보이기 시작했다(D1).** 실플레이 3판이
끊김 없이 이어졌다 — 각 판의 `mmrBefore`가 직전 판 `mmrAfter`와 정확히 일치. 남은 것은 프로필 전적(D2)뿐.
...
| **D2** | 프로필 전적 (판수·1등·평균 등수·현재 MMR) | 프로필 셸(`ProfileView.cs`, 8줄)을 채운다. 조회 라우트는 A에서 열렸고 클라 스토어(`userRatingByQueueId`)도 이미 채워진다 — **표시만 남았다** |
