...
판단 기준은 **"이 동작이 끝나면 View+VM 쌍이 교체되는가?"** 다.

- **작은 흐름**(View+VM 유지, 화면 안 상태·값만 변함) → **ViewModel**이 담당.
- **큰 흐름**(View+VM 교체 = 네비게이션) → **코디네이터**가 담당. 씬 교체는 게임 상태 머신(FSM), 윈도우 교체는 윈도우 매니저.
- VM은 큰 흐름을 직접 실행하지 않고 **도메인 신호만 노출**한다(예: "로그인 성공" — 목적지는 모름). 일회성 종료 결과는 `UniTask`, 연속·반복 신호는 `Observable`. 외부 코디네이터가 그 신호를 듣고 화면을 교체한다.
...
### 업계 표준 구조·네이밍 — 임의 명명 금지 (필수)

새 **타입·인터페이스·멤버·facade·폴더 구조**를 도입할 때는 *떠오르는 대로 짓지 말고*, 그 개념의 **확립된 업계·생태계 표준 용어와 구조를 먼저 확인해 따른다.** (위의 Microsoft C# 컨벤션은 *표기 스타일*[PascalCase 등]만 다룬다 — *도메인 명명·구조 선택*은 이 규칙이 강제한다.)

- **확정 전 체크 (필수 — 단 *대부분 이미 아는 지식*으로 끝, 매번 웹 검색 아님)**: "이 개념을 관련 프레임워크(Unity/Mirror/NGO/Netcode-for-Entities/FishNet/Photon/Unreal 등)·도메인은 **뭐라고 부르나**? 표준 **구조**는?"를 *짓기 전에 의식*한다. 보통은 *알고 있는* 생태계 관용어/패턴을 적용 + 코드베이스 짝 일관을 맞추면 끝난다.
  - **외부 리서치(웹/프레임워크 소스·문서 확인/사용자 확인)는 에스컬레이션**: ① 표준 용어가 *정말 불확실*하거나 후보가 충돌할 때, ② 공개 API·핵심 추상처럼 *틀리면 rename 비용이 큰* 경우에만. 핵심은 *"짓기 전에 표준을 의식하는 습관"* 이지 매번 검색이 아니다.
- **기존 프레임워크 개념의 추상화·래핑은 그 이름에 맞춘다**: 예 — Mirror `NetworkTime`을 추상화 → `INetworkTime`/`GameEngine.NetworkTime`(NGO도 `NetworkTime`/`NetworkTimeSystem`). **임의 명명(`IClock` 등)으로 시작했다가 나중에 표준으로 되돌리는 rename churn을 만들지 않는다.**
- **기존 코드베이스와 짝 일관**: 새 이름은 인접/대응 식별자와 짝이 맞아야 한다(예: `GameEngine.Time` ↔ `GameEngine.NetworkTime`). 한 코드베이스 안에서 같은 개념에 다른 어휘를 섞지 않는다.
- **구조도 표준 매핑 우선**: 새 구조·패턴은 *새 발명보다* 검증된 업계 표준 조립(이 문서·도메인 설계 문서가 채택한 패턴)을 따른다.
- **근거 기록은 결정 시점에**: 비자명한 구조·네이밍 결정은 해당 spec/설계 문서에 **"산업 표준 매핑"** 절로 "어느 프레임워크의 무엇에 대응하는지"를 남긴다 — *사후 정당화가 아니라 결정 시점의 확인 절차*다.
