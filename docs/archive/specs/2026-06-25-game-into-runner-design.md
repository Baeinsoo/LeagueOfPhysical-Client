...
`docs/archive/specs/2026-06-06-game-scene-scope-design.md`가 제안한 "Game을 별도 씬+Room 자식 스코프로"는 **이미 코드에 구현돼 있다**: Game은 "LOPGame" 씬에 살고, `LOPGameFactory`가 Room 자식 스코프로 동적 로드. → **③는 씬/스코프 경계를 바꾸지 않는다.** 그 씬 안의 *두 형제 MonoBehaviour(`LOPGame`+`LOPRunner`)를 하나로 합치는* 작업이다. 씬 이름("LOPGame")·스코프명(`GameLifetimeScope`)은 "게임플레이 씬/스코프"로 유지.
...
- [x] **2단계(Game→Runner 흡수) 구현 완료**(2026-06-26): `IGame` 삭제, `IRunner`가 `gameState`/`onGameStateChanged` 흡수(보관·발화는 `RunnerBase`로 일반화, 구체 상태값 전이만 `LOPRunner`), `RunnerBase.Run/Stop` virtual, `IRoom.runner`/`IGameFactory:Task<IRunner>`/`IGamePresenter:IRunner` 경계 교체. `LOPGame`(클·서) 삭제 + 씬에서 컴포넌트/직렬화 참조 제거. 클·서 컴파일 0 + 런타임 검증 OK. **⚠️ 발견·해소한 순환:** Game을 Runner에 합치니 `Runner→GameRuleSystem→IRunner` / `Runner→gameMessageHandlers→IRunner` VContainer circular dependency(런타임 DI 빌드)가 생김(예전엔 중간 `LOPGame`이 사이클을 끊고 있었음). 해소 = ①룰은 호스트 역참조 제거(`IRunner`→`IEntityManager`+`ITickUpdater`, 후자는 GameLifetimeScope 팩토리 등록; 룰은 runner-init+맵로드 후 타이밍 필요라 Runner-driven 유지), ②**메시지 핸들러 구독 생명주기를 `GameLifetimeScope`(컴포지션 루트)가 소유**(build-callback에서 resolve+Register, OnDestroy에서 Unregister) → Runner가 핸들러를 아예 참조 안 해 사이클 소멸. 서비스 로케이터(처음엔 `IObjectResolver` lazy resolve로 막았다가 사용자가 "땜빵 아니냐" 지적 → 핸들러 생명주기는 세션 스코프 소유가 맞다로 재구성) 제거. 핸들러는 그대로 `IRunner` 주입(단방향), 클·서 대칭, behavior-identical(스코프빌드~runner-init 사이엔 네트워크·틱 없음). 서버 `Game.Info`만 `runner.AddListener`로 IRunner가 진짜 필요. **⚠️ 씬 교정:** 플랜은 presenter↔runner colocate를 가정했으나 실제 씬은 presenter=부모 GO("LOPGame")/LOPRunner=자식 GO("LOPGameEngine") → `LOPGamePresenter`를 `GetComponentInChildren<LOPRunner>()`로 변경.
