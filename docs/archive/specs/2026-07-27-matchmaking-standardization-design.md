...
### 슬라이스 4b 확정 사항 (착수 시점 2026-07-31에 코드를 열어 보고 정한 것)

#### Director 루프 — 겹치지 않게 스스로 예약한다

`setInterval`이 아니라 **한 틱을 끝까지 await한 뒤 다음 틱을 `setTimeout`으로 예약**한다. `setInterval`은
한 틱이 주기를 넘기면 다음 틱이 겹쳐 시작하고, 겹치면 같은 티켓이 두 매치에 들어간다. replica 1과
이 모양을 합치면 **매칭 루프가 동시에 두 번 도는 일이 구조적으로 불가능**하다. `SIGTERM`을 받으면
진행 중인 틱을 끝내고 다음을 예약하지 않는다.

기존 `Updater`(1초 tick + 등록/해제되는 `Updatable` 집합)는 백엔드에 게임 루프를 흉내 낸 것이라 삭제한다.

#### 매치 확정의 커밋 지점 — "풀에서 뺐다"와 "매치가 있다"를 한 트랜잭션으로

현재 `updateWaitingRoom`은 매치 생성 → 룸 생성 → 티켓 삭제 → 위치 갱신이 전부 따로라, 중간에 실패하면
어떤 상태가 되는지 정의돼 있지 않다. Director는 이렇게 한다:

| 순서 | 무엇 | 실패 시 |
|---|---|---|
| **1** | **매치+라운드 생성과 티켓 삭제를 한 DB 트랜잭션** | 전부 원복. 티켓이 풀에 남아 다음 틱에 재시도 |
| 2 | 룸 생성 (HTTP) | 유저는 이미 풀에서 빠짐 → 위치가 `WaitingRoom`인데 티켓이 없으므로 **로비 자가치유가 `None`으로 되돌림**. 매치 행만 고아로 남음 |
| 3 | 유저 위치 갱신 (HTTP) | 위와 같음. 룸은 좀비로 남아 타임아웃 |

**이 시스템에서 가장 나쁜 고장은 한 명이 두 매치에 들어가는 것**이므로, 그것만은 구조적으로 막고
나머지는 "매치를 잃되 로비로 안전하게 돌아간다"로 퇴화시킨다. 2·3은 서로 다른 서비스로 나가는 HTTP라
DB 트랜잭션으로 묶을 수 없다(슬라이스 2에서 미룬 "넓은 경로"가 이것이며, 여기서 *경계를 정하는 것*으로
결론 낸다 — 분산 트랜잭션은 도입하지 않는다).

#### 티켓 생성 시 검증 — "조용히 영원히 대기"를 없앤다

지금은 잘못된 요청도 티켓이 만들어지고 아무 일도 일어나지 않는다. `requestMatchmaking`에서 거절한다:

- 없는 `queueId`
- 그 큐의 `allowedGameModeIds`에 없는 게임
- 그 게임에 속하지 않는 맵
- **파티 인원 > 그 게임의 정원** — 현재 티켓은 항상 1명이라 도달 불가능하지만, 이 가드가 있어야
  "매칭 풀에 들어간 티켓은 언젠가 반드시 매칭될 수 있다"가 참이 된다

#### 로비 자가치유 — 티켓 존재만 본다

로비의 `verifyUserLocation`이 "대기방이 있나 → 그 대기방에 내 티켓이 있나"를 보던 것을 **"티켓이 있나"**
하나로 줄인다. 로비의 `waitingRoom.service.ts`와 매칭서버 대기방 조회 HTTP 호출이 사라진다.
`locationDetail` 판별자도 `waitingRoomId` 존재 → `matchmakingTicketId` 존재로 바꾸고 `waitingRoomId`
필드를 제거한다. **클라는 `location` enum 값으로 판별하고 `waitingRoomId`를 읽는 코드가 0곳**이라 무영향.

#### `Location.WaitingRoom`은 이번에 개명하지 않는다

DB enum 이름 변경은 마이그레이션 + 5개 저장소를 건드리므로 §8 표대로 **슬라이스 5**에 둔다. 4b에서는
**뜻만 바뀐다** — "대기방에 있음" → "매칭 풀에서 기다리는 중".

#### Casual `max_wait_seconds` 30 → 10

현재 코드는 대기 시간이 `5`로 하드코딩돼 있어 엑셀 값이 쓰이지 않았다. 4b에서 값이 실제로 쓰이기
시작하므로 함께 조정한다(infrastructure 엑셀 → `gen.sh` → 매칭서버 `master_data/` + MasterData-Client/Server).
**대기 시간이 늘어난다는 뜻이 아니다** — 정원 8·최소 2면 10초에 걸쳐 필요 인원이 8→2로 줄고,
사람이 모이면 그 전에 출발한다(§6-2 정정).

#### 관측 — 못 맞는 티켓과 풀 크기

Director가 매 틱 조용히 도는 것만으로는 "아무도 매칭이 안 된다"와 "대기자가 없다"를 구분할 수 없다.
로그로만 낸다(지표 인프라는 도입하지 않는다):

- 매치를 만든 틱: 만든 매치 수·게임·인원
