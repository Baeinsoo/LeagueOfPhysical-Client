...
- **스폰 지점을 서버가 읽게 한다.** `PlayerSpawn_1~4`가 있으나 **비활성**(`m_IsActive: 0`)이고
  ...
  - **마커는 켤 필요가 없다(최종 리뷰 Finding A, 2026-08-22 정정).** 서버 룰은
    `FindObjectsInactive.Include`로 찾으므로 마커는 **비활성 그대로** 둔다. 처음 수술 도구가
    `SetActive(true)`로 켰다가, 각 마커의 자식 장식(`Halo`, `Bird_PN` 프리팹 인스턴스)까지
    함께 켜져 출발선에 가짜 새 네 마리가 보이는 것을 발견하고 되돌렸다. `SpawnPoint`/`Order`는
    그대로 둔다 — 자식을 지우는 대신 이 방법을 쓰면 에디터에서는 마커가 그대로 보여 맵 작업자가
    자리를 알아볼 수 있고, 런타임에만 안 그려진다.
...
3. **대신 진짜 어긋난 곳이 있다 — 몸 규격이 두 곳에 다른 값으로 있다.** (→ B2-d2 **첫 항목**,
   최종 리뷰 Finding E로 승격)
   ...
   어긋나면 깨진다"가 프리팹이 아니라 **코드**에 실재한다.

   ~~지금은 `Depenetrate`(지형에 박힌 것 빼내기)와 *남의* sweep이 이 몸을 볼 때만 영향이 있어
   급하지 않다.~~ **그 문장은 맵이 전부 트리거이던 시절에 쓴 것이고, B2-d1이 그 전제를
   없앴다.** `FlappyWorld.MoveThroughMap`은 sweep 앞에 `_motionBridge.Depenetrate(entity)`를
   부르고, 그건 `OverlapCapsule(..., QueryTriggerInteraction.Ignore)`로 간다 — 콜라이더가
   트리거인 동안엔 맵에 대해 아무 일도 안 했지만, B2-d1에서 맵 콜라이더를 솔리드로 바꾼
   지금은 **매 틱 실제로 동작한다.** 그때 쓰는 캡슐은 물리 팔로워 것(높이 1.5)이라 시뮬 몸
   (높이 0.9)보다 **위로 0.6m 더 크다** — 새가 파이프 아랫면에 닿으면 sweep은 아무것도 못 봤는데
   `Depenetrate`가 트랜스폼을 아래로 밀어낸다. 시뮬이 모르는 위치 점프가, 속도는 그대로인 채,
   천장을 스치는 매 틱마다 일어난다. **더 이상 급하지 않은 문제가 아니다** — d2 착수 시
   가장 먼저 통일한다.

### B2-d1의 대표 확인은 오늘 채점할 수 없다 (최종 리뷰 Finding D, 2026-08-22)

첫 콜라이더가 x=16이고 스폰은 x=-2다. 전진 11로 1.64초가 걸리는데 **플랩할 수단이 없어서**
(중력 70, 최대 낙하 30) 그 사이 40m 넘게 떨어진다. 코스는 그 지점에서 y −10~+10이라 새는
아래로 통과해 버린다. 맵의 유일한 바닥(`Ground`)도 비활성이라 받아 줄 것도 없다. 그래서
d1의 런타임 검증은 나눈다:

- **d1에서 지금 채점 가능**: 파드 로그에 `missing script` **0건**(전 588건), `InjectSceneObjects`
  NRE **0건**, 새가 x≈−2(마커 자리)에서 스폰.
- **B2-d2로 이동**: "코스의 틈을 통과하는지" — 플랩 수단이 생겨야 볼 수 있다. 아래 §6-2 끝의
  "런타임 검증"과 §7 표의 "나는 감각·충돌"이 이 항목이다.
  - *(2026-08-23 갱신)* "지형에 **막히는지**"는 d1에서 관측됐다 — 아래 "결과"의 런타임 검증 참고.
    d2로 남는 것은 *난다*(플랩으로 떠서 틈을 지난다) 쪽이다.
...
| 저장소 | main | 담긴 것 |
|---|---|---|
| LOP-Shared | `44ad13a` | `SpawnPoint` 마커 + `SpawnPlacement.Arrange` + 테스트 5개 |
| **LeagueOfPhysical-Art** | `43c2453` | 정리된 `FlappyRaceMap.unity` |
| LOP-Client | `5c43417` | 서브모듈 포인터, 씬 회귀 테스트, 주석 정정 |
| LOP-Server | `fb81eca` | 룰이 스폰 마커를 읽는다 |

배포: `gameserver-deploy` 성공 → infra가 `GAME_SERVER_IMAGE -> fb81eca`로 자동 bump →
ArgoCD 롤아웃. `content-deploy -f target=gameserver` 성공.

**테스트**: 클라 EditMode 527/527, 서버 506/506. 새로 넣은 테스트는 하나하나 *일부러 깨뜨려
실패하는 것*을 확인한 뒤 되돌렸다.
...
|---|---|---|
| GameObject | 228 | 217 |
| MonoBehaviour | **147** | **4** (전부 `SpawnPoint`) |
| MeshFilter(코스 기하) | 188 | **188** — 하나도 안 잃음 |
| Camera / Light | 1 / 1 | 0 / 0 |
| BoxCollider | 119 | 119 |
| 그중 트리거 | **119** | **0** |
| 스폰 마커 | 4개 전부 꺼짐 | 4개, `Order` 1~4 |

사라진 오브젝트 11개 = 프로토타입 액터 10 + Directional Light. 새로 생긴 것 0.
...
| 확인 | 결과 |
|---|---|
| `The referenced script (Unknown) ... is missing!` | **0건** (§3에서 588건) |
| `InjectSceneObjects` NRE | **0건** |
| `NullReferenceException` 전체 | **0건** |
| `SpawnPoint가 없다` 폴백 경고 | **0건** → 서버가 마커를 찾아 썼다 |

그리고 **새가 지형에 막혀 멈췄다.** 클라가 받은 상태는 `pos=(14.73, -9.98, 0)`,
`vel=(0,0,0)`으로 20초 넘게 정지인데 러너는 `Playing`이고 틱은 계속 올랐다(4716→5171) —
시뮬이 도는데도 안 움직인다는 뜻이다. 그 자리를 프로브한 결과:

- 겹친 콜라이더: `Cube` ×2 (layer 0 Default, **`isTrigger=false`**) + `Actor_1` (layer 8 Character = 새 자신)
- 전진(+X) sweep: `Cube`에 거리 **0.020**, 법선 `(-1, 0, 0)` — 수직 벽면이 앞을 막고 있다
- 아래 sweep: `Cube`에 거리 **0.020**

`0.020`은 `KinematicMover.SkinWidth`와 **정확히 같은 값**이다. 서버의 이동 커널이 "벽 앞
이만큼까지만"이라고 계산한 그 거리에 새가 서 있다 — 두 방향 다 우연히 맞을 수는 없다.
슬라이스 전이라면 콜라이더가 전부 트리거라 sweep이 걸러내고 그대로 통과했을 자리다.

덤: 프로브에 잡힌 `Actor_1`이 layer 8(`Character`)이라, 아래 "정정 2"(새는 이미 Character
레이어라 sweep 마스크가 지금도 맞다)가 런타임에서도 실증됐다.

#### 검증의 한계 (정직하게)

- **"코스의 틈을 통과하는지"는 여전히 못 본다.** 플랩할 수단이 없어(§6-2) 새는 뜨지 못하고
  바닥에 내려앉아 미끄러지다 벽에 낀다. *막힌다*는 것은 확인됐지만 *난다*는 것은 d2의 몫이다.
- 마지막 리뷰는 "첫 콜라이더가 x=16이고 그 전에 40m 넘게 떨어져 코스 아래로 통과하므로 오늘은
  채점 불가"로 판단했는데, 실제로는 훨씬 일찍 바닥에 내려앉았다(y≈-10에서 미끄러져 x=14.73까지).
  리뷰어의 "x=-2~16 사이엔 콜라이더가 없다"는 측정과 어긋난다. **어느 쪽이 맞는지는 파고들지
  않았고**, 관측된 사실만 남긴다.
- 클라는 아직 자기 새를 시뮬하지 않는다(`Simulated` 없음 — d2). 위 위치는 전부 *서버가 보낸 것*이다.

#### 실행하며 배운 것 (다음 사람을 위해)

- **로컬에서 방 접속이 `Connection refused`면 도커가 UDP 7000만 안 잡은 것이다.** 7001~7009는
  잡혀 있고, `docker port`는 7000도 잡았다고 *주장하지만* `lsof -nP -iUDP:7000`은 0줄이다.
  `docker restart lop-control-plane`으로 고친다. B2-c에 이어 두 번째 발생.
  - **UDP를 프로브할 땐 반드시 `connect` 후에 보낼 것.** 연결하지 않은 소켓은 ICMP 거절을 못 받아
    "무응답"으로 보이고, 그걸 "정상"으로 오독하게 된다(이번에 실제로 한 번 오독했다).
  - 헷갈리게 하는 것들: 파드의 `hostPort` 선언, kind 노드의 DNAT 규칙, 파드 로그의
    `Server listening on port 7000` — 셋 다 정상인데도 접속이 안 된다.
- **`LOPRoom.ConnectRoomServerAsync`에 재시도가 없다** — `StartClient()` 한 번 하고
  `NetworkClient.ready`를 무한정 기다린다. 서버 소켓이 열리기 전에 붙으면 거기서 멈춘다.
  로컬은 파드가 뜨는 데 시간이 걸려 이 경합에 자주 걸린다. (이 슬라이스와 무관한 기존 약점.)
- **씬 검증은 저장된 YAML로.** 800KB짜리 씬을 손으로 고치지 말고 일회용 에디터 도구를 만들어
  돌린 뒤 지운다. 검증은 파일을 파싱해서 한다.
- **서브모듈은 두 번 커밋한다** — 아트에 씬을, 클라에 포인터를. 아트를 머지해 main 끝이 머지
  커밋이 되면 포인터를 **그 지점으로 다시 맞춘 뒤** 클라를 머지해야 한다.
- **배포는 두 워크플로다** — `gameserver-deploy`(서버 코드)와 `content-deploy`(맵 콘텐츠).
  콘텐츠만 돌리면 "맵은 깨끗한데 새는 원점 스폰"이 되고, 증상이 마커 조회 실패처럼 보인다.

---

## 6-2. B2-d2 — 클라가 자기 새를 난다

- **[첫 항목, 최종 리뷰 Finding E로 승격] 몸 규격을 한 곳으로 통일한다** — 위 §6 "정정 3".
  `PhysicsFollower`가 게임별 캡슐 치수를 받게 하고, Flappy는 `TbFlappyConfig`의
  `body_radius`/`body_height`를 넘긴다. **B2-d1 전에는 "급하지 않다"고 적었으나 더는 사실이
  아니다** — 그때는 맵 콜라이더가 전부 트리거라 `Depenetrate`(맵에 대해
  `OverlapCapsule(..., QueryTriggerInteraction.Ignore)`로 겹침 검사)가 트리거를 걸러 사실상
  아무 일도 안 했다. B2-d1이 맵 콜라이더를 솔리드로 바꾸면서 그 전제가 사라졌다 — 이제
  `Depenetrate`가 물리 팔로워 몸(높이 1.5, 시뮬 몸 0.9보다 위로 0.6m 큼)으로 맵과 매 틱
  실제로 겹침 검사를 하고, 새가 파이프 아랫면을 스칠 때마다 시뮬이 모르는 위치 점프를
  트랜스폼에 밀어넣는다. d2 착수 시 다른 항목보다 먼저 처리한다.
...
| 저장소 | 담긴 것 |
|---|---|
| GameFramework | `CapsuleShape` 컴포넌트(몸 캡슐 치수) + 테스트 3개 |
| LOP-Shared | `FlappyWorld`·`KinematicMoveSystem`이 엔티티의 몸을 읽는다 + `BodySizes` 상수 + 테스트 2개 |
| LOP-Client | 크리에이터 3개가 몸을 붙인다 · `PhysicsFollower`가 그 몸으로 캡슐을 세운다 · 내 새에 `Simulated` · 날갯짓 화면(`FlapPadView`) 신설 |
| LOP-Server | 크리에이터 3개 + `PhysicsFollower` (클라와 대칭) |

**테스트**: 클라 EditMode 532/532, 서버 511/511. 새로 넣은 테스트는 하나하나 *일부러 깨뜨려 실패하는
것*을 확인한 뒤 되돌렸다.
...
| | 전 | 후 |
|---|---|---|
| `PhysicsFollower`(클·서) | 0.35 / 1.5 하드코딩 | 엔티티의 `CapsuleShape`를 읽는다(없으면 즉시 예외) |
| `KinematicMoveSystem` | 상수 0.35 / 1.5 | 엔티티의 `CapsuleShape` |
| `FlappyWorld` 맵 sweep | `FlappyConfig`(0.45 / 0.9) | 엔티티의 `CapsuleShape` |
| 값의 출처 | 세 곳이 각자 | 크리에이터가 붙인다 — 새는 `TbFlappyConfig`, 캐릭터·아이템은 `BodySizes` |

`FlappyBodyCollisionSystem`(새끼리 몸싸움)만 아직 `FlappyConfig`를 읽는다 — 짝별 치수를 받으려면
테스트가 붙은 순수함수 `FlappyBodyOverlap.TryCompute`의 시그니처를 바꿔야 해서 미뤘다. 값의 출처가
같아 지금은 어긋날 수 없지만, **"몸 치수는 엔티티가 갖는다"의 유일한 예외**로 남아 있다.

`CapsuleShape`는 생성자에서 `height ≥ radius×2`를 강제한다. 그보다 낮으면 Unity `CapsuleCollider`는
높이를 지름으로 클램프해 *구*가 되고 `KinematicMover`는 뒤집힌 캡슐을 만들어, **이 슬라이스가 고친
바로 그 어긋남이 되살아난다.** 현재 튜닝값(0.45 / 0.9)이 정확히 그 경계에 있어 다음 밸런싱에서
높이를 조금만 낮추면 걸리는 자리다 — 그래서 경고가 아니라 생성 시 예외로 막았다.

#### 런타임 검증 — 목적이 관측됐다

로컬 클러스터에 서버 이미지(`2d17187`)를 올리고 사람이 직접 플레이했다:

| 확인 | 결과 |
|---|---|
| 화면 탭 / Space | 새가 뜬다 |
| 안 누를 때 | 떨어지고, 바닥에서는 막힌 채 미끄러진다(collide-and-slide) |
| **코스의 틈** | **통과한다** — B2-d1이 "플랩 수단이 없어 오늘 채점 불가"로 남긴 그 항목 |
| 파이프·바닥 | 막힌다(뚫고 가지 않는다) |
| FlapWang 회귀 | 없음(걷기·점프·대시 감각 그대로) |

#### 검증의 한계 (정직하게)

- **클라는 새끼리 몸싸움을 구조적으로 예측하지 않는다 — 이 슬라이스가 만든 갈림이다.**
  `FlappyWorld.CollectBirds`가 `Simulated`만 모으는데 클라에서 그건 내 새 하나뿐이라, 몸싸움
  이중 루프가 짝을 하나도 만들지 못한다. 맵 sweep 마스크도 `Default`뿐이고 `Separate`도 부르지
  않으므로 **남의 새는 클라 시뮬에 어떤 형태로도 존재하지 않는다.** 결과: 몸이 겹치는 동안
  클라에서는 내 새가 상대를 통과하고, 서버 스냅마다 하드 보정이 걸린다(육안으로는 자리싸움 중
  러버밴딩). **버그가 아니라 현재 구조의 정상 동작이다** — 다음에 이걸 다시 파지 않도록 적어 둔다.
  - FlapWang은 다르다: `LOPWorld`는 `Separate`를 부르고 원격 캐릭터의 콜라이더가 Character
    레이어에 실재해 클라도 밀어냄을 예측한다. Flappy만 몸싸움을 월드 코드(`_birds` 목록)로 풀기
    때문에 `Simulated` 게이팅이 그대로 "상대가 없음"이 된다.
  - 고치는 형태(다음 슬라이스): `CollectBirds`를 **이동 대상(`Simulated`)** 과 **충돌 참가자(전원,
    원격은 부동)** 로 가른다. 그때 `FlappyBodyOverlap`도 엔티티 `CapsuleShape`를 읽게 옮긴다.
- **몸싸움 런타임 미검증.** MPPM 가상 플레이어로는 새 두 마리를 서로 부딪히게 만들기가 어려웠다.
  현재 커버는 B2-b의 EditMode 8개 테스트뿐이다. 위 항목 때문에 실기로 봐도 "예측 없음 + 보정"이
  보일 것이므로, 이 검증은 다음 슬라이스에서 클라 몸싸움 예측과 함께 하는 것이 맞다.
- **디버그 HUD가 깔린 자리(화면 오른쪽 위)는 탭이 먹지 않는다.** 같은 밴드에서 나중에 열려 위에
  깔리기 때문. FlapWang에도 있는 기존 성질이라 이번엔 두었다.

#### 실행하며 배운 것 (다음 사람을 위해)

- **`gameserver-deploy`는 형제 패키지 레포를 *러너 자신의 클론*에서 `reset --hard @{u}`로 맞춘다.**
  그 클론들은 `main`을 보고 있어서, 피처 브랜치의 패키지 변경은 CI에 보이지 않는다(컴파일 에러로
  드러난다). 검증을 위해 러너 클론을 그 브랜치로 체크아웃했다면 **끝나고 반드시 main으로 되돌릴 것** —
  안 그러면 다음 main 배포가 조용히 피처 코드를 굽는다.
- **유니티 CLI 명령은 버전마다 다르다.** 이 환경에는 `refresh_unity`가 없고
  `import_asset`/`recompile`/`recompile_status`/`run_tests`/`test_status`가 있다. 배치모드
  `unity test`는 그 프로젝트의 에디터가 떠 있으면 못 쓴다.
- **대상 프로젝트의 에디터가 안 떠 있으면 `unity cmd`가 조용히 다른 인스턴스로 라우팅된다**(에러 없음).
  서버 쪽을 확인할 때 클라 결과를 서버 결과로 착각하기 쉽다 — 배치모드로 돌리거나 `unity status`로
  확인할 것.
...
- `docs/archive/specs/2026-08-15-game-mode-axis-design.md` — 게임 모드 축 전체, B1/C 결과
...
- `docs/archive/specs/2026-07-09-shared-kinematic-character-controller-design.md` — `KinematicMover` 커널
