...
- **정책의 집** — **클라 `Assets/Scripts/EntitySync/`, 자기 asmdef(`LOP.EntitySync`)를 가진다.**
  처음엔 LOP-Shared에 두었는데(단위 테스트를 붙이려면 asmdef가 필요했고 클라 코드는 전부 `Assembly-CSharp`이라
  테스트 어셈블리가 참조할 수 없었다), **그건 배치 근거가 될 수 없다** — 이 개념은 클라 전용이고
  서버에는 "원격을 어떻게 따라갈까"라는 질문 자체가 없다. 유니티 설계상 테스트 asmdef가 `Assembly-CSharp`를
  참조하는 길은 없고 공식 해법은 "프로덕션 코드를 asmdef로"뿐이므로, **작은 asmdef를 주어 레이어와 테스트를
  둘 다 지켰다**(저장소 선례: `FlappyRaceSlice.Logic`). 테스트도 클라 `Assets/Tests/EditMode/EntitySync/`로 옮겼다.
...
- ~~정책이 공유 패키지에 산다~~ → **해소.** 클라로 옮기고 `LOP.EntitySync` asmdef를 주었다(위 참조).
  다만 그 과정에서 드러난 구조적 원인은 남는다 — **클라 코드가 asmdef 레이어링을 쓰지 않아서**
  (`Assets/Scripts` 전체가 `Assembly-CSharp` 한 덩어리) "테스트를 붙이려면 패키지로 밀어 넣게 되는" 압력이
  반복된다. `architecture-guidelines.md`는 레이어별 asmdef를 이미 규정하고 있는데 클라는 적용된 적이 없다.
  `docs/ROADMAP.md`의 파킹 표에 기록했다.
...
- `docs/archive/specs/2026-08-17-flappy-race-gameplay-b2-design.md` §6-2 — 이 문제가 드러난 자리
