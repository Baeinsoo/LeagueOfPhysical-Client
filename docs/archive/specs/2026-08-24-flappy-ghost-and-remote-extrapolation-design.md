...
- [ ] **(알려진 한계) 내 새의 유령 상태는 서버로 되먹여지지 않는다** — `FlappyRaceLifetimeScope`가
      `NoServerCorrection`을 등록해 두어서(위치 보정 자체를 안 함) `snap.ghost`가 write-only다:
      클라·서버가 "부딪혔다"를 다르게 판단하면 0.8초 정지 여부가 서로 갈리고, 위치 보정이 없으니
      고칠 방법도 없다. 지금은 판정이 결정론적(같은 입력·같은 충돌질의)이라 감수했다. **재검토 트리거:**
      유령정지 판정에 비결정 요소(예: 서버만 아는 지연 보상, 서버 전용 랜덤)가 들어가거나, 실전에서
      클·서 유령 판정이 갈리는 사례가 관측되면.
- [ ] **(알려진 한계) 되감기 재생이 원격 새의 "지금" 위치로 몸싸움을 계산한다** — `_bodies`(몸싸움
      상대 목록)에는 원격(Simulated 아닌) 새도 들어가는데, `WorldBase.SaveState`/`LoadState`는
      Simulated 엔티티의 위치·속도만 저장·복원한다. 그래서 되감기 재생 중에는 원격의 위치가 그 틱
      당시 값이 아니라 재생을 시작한 현재 프레임 값 그대로 고정된다 — 재생 중 몸싸움 판정이 원격의
      과거 위치가 아니라 현재 위치를 본다. **재검토 트리거:** 몸싸움 결과가 프레임 하나 차이로 크게
      갈리는 상황(예: 판정 프레임 근처에서 원격이 빠르게 움직이는 콘텐츠)이 생기면, 원격 모션도
      SaveState 대상에 넣는 걸 검토한다.
