...
### 5.2 결정 — 동전당 임펄스 하나 + 덮임 가중

> ⚠️ **이 절은 슬라이스 3 설계에서 두 번 정정됐다.** 확정 내용과 근거는
> [`2026-08-24-panchigi-slice3-strike-design.md`](2026-08-24-panchigi-slice3-strike-design.md) §4.4에
> 있다. 아래는 요지만 남긴다.

접촉점 수천 개에 `AddForceAtPosition`을 넣는 대신, **동전마다 임펄스 하나**로 간다.

**정정 ① — 원본은 이미 임펄스 하나였다.** 원본이 접촉점마다 준 부분 힘은 전부 *같은 지점*
(`forcePos`)에 걸린다. 그러므로 합이 `AddForceAtPosition(force x Sum(falloff), forcePos)`와
**수학적으로 같다.** 격자는 회전을 만든 게 아니라 **스칼라 배수 하나**를 만들고 있었다(그래서
`gridDivisions`가 세기까지 바꿨다). 회전은 원래부터 중심에서 어긋난 지점에 힘을 준 데서 나온다.

**정정 ② — 그 스칼라에는 버릴 수 없는 정보가 있다.** `Sum(falloff)`는 *"동전이 판에 실제로 닿은
면적을 타격점까지의 거리로 가중한 값"* 이다. 격자가 존재한 진짜 이유가 이것이고, 두 경우를 담는다:

- **판 끄트머리에 반만 걸친 동전** → 힘이 작다 (원본에서 동작)
- **다른 동전 위에 포개진 동전** → 힘이 없어야 한다 (**원본은 버그로 동작하지 않음** — 위로 쏜
  레이가 맞은 게 *누구인지* 확인하지 않아, 아래 동전을 맞은 점이 위 동전 몫으로 들어간다)

그래서 단순 "중심 거리 falloff"로 대체하면 **두 경우를 다 잃는다.** 대신 동전 발자국에 **고정
개수 K개 샘플**을 깔고, 판 밖·포개짐을 걸러 낸 뒤 `Sum(falloff)/K`를 **덮임(0~1)** 으로 쓴다.

얻는 것:

- 레이캐스트 16,000회 x 매 물리 스텝 → **K x 동전수, 타격당 1번** (약 128회)
- K가 상수라 **세기 노브와 정밀도 노브가 분리**되어 튜닝이 가능해진다
- 원본의 포개짐 버그가 고쳐진다
- 계산이 **순수 함수**가 되어 LOP-Shared에 놓고 EditMode 테스트를 붙일 수 있다

감각은 유지된다 — 타격점에서 멀수록 약하게, 중심에서 어긋날수록 많이 돈다. 적용 지점은 원본대로
타격점이다.
...
| **3** | 타격 입력 + 힘 커널(원본 물리 재작업) — **3-0 물리 포트 정리 / 3-1 본편**. 상세: [slice3 spec](2026-08-24-panchigi-slice3-strike-design.md) | 칠 수 있고 동전이 튄다 |
