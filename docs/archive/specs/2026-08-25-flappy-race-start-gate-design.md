...
- 내부: `docs/netcode-redesign.md` §4~5 (clock sync, 입력 버퍼), `docs/world-core-connection-architecture.md`
  (Engine↔Simulation 책임 분리), `docs/archive/specs/2026-08-24-flappy-ghost-and-remote-extrapolation-design.md`

---

## 14. 실측 결과 (2026-08-25, 2인 라이브)

에디터 서버 + 메인 클라 + MPPM 가상 플레이어로 실제 붙여서 확인했다.

### 확인됨

| 무엇 | 관측 |
|---|---|
| 준비 인원 집계 | `1 / 2 대기 중` → `2 / 2` |
| 출발 전 새 정지 | 두 번째가 붙기까지 첫 새가 스폰 지점에 그대로 |
| 카운트다운 | `3 → 2 → 1 → GO!`, 각 약 1초, **`0`은 나오지 않음** |
| GO와 새 | 같은 순간에 움직이기 시작 |
| 클·서 출발틱 일치 | 양쪽 `GameplayStartTick=5644` (eval로 동시 조회) |
| 입력 전송로 | 시퀀스 12,641개 도착, prune 0, seq-gap 0 |
| 매치 시계 재기준 | 출발 후 5분에 판이 스스로 종료 (방 부팅 기준이었다면 훨씬 일찍 끝났을 것) |
| 지각 입장자 체감 | 정착 2초 도입 + 대기 문구 표시로 "갑자기 3부터" 해소 |

**최종 리뷰가 잡은 Critical(메시지 브로커·디스패처 미등록)이 런타임으로 고쳐졌음이 증명됐다** —
준비 신호가 서버에 도달하지 않았다면 대기 인원이 0에서 움직이지 않았을 것이다.

### 못 한 것 — 두 새를 화면에 붙여 둘 수 없다

유령정지 연출(G2)·원격 새 매끄러움(G3)·몸싸움 정합·카메라 진동은 **확인하지 못했다.**

두 새가 나란히 있어야 하는데 그 상황을 만들 수단이 없다:
- 자동조종 봇이 파이프를 못 피해 유령정지(0.8초)를 반복해서 먹고, 전진 속도는 일정하므로 **영영 뒤로 밀린다.**
- 사람은 두 클라를 동시에 조종할 수 없다.

**다음 슬라이스의 선행 조건**이다 — 피할 줄 아는 봇이든, 맵 충돌 정책 변경이든.

### 라이브에서 새로 발견 — 외삽이 게임 규칙을 모른다

같은 새의 수직 속도가 서버(시뮬) `-30`, 클라(외삽) `-35.67` → 잠시 뒤 `-39.5`로 **계속 벌어졌다.**
시뮬은 `MaxFallSpeed`에서 자르는데 외삽 커널은 `v + a·t`를 그대로 쓴다.

`FlappyBounce`가 부딪힐 때 세로 속도를 주고받으므로 이 오차가 몸싸움 힘에 직접 들어가고,
`NoServerCorrection`이라 고쳐지지 않는다. 상세와 선택지는 `docs/ROADMAP.md`의
"Flappy 몸싸움 정합성" 절에 기록했다.

### 검증 리그에서 걸린 것 (게이트와 무관)

- MPPM 클론이 **재시작할 때마다 새 익명 계정**을 받는다(자격증명 저장이 깨져 있음). 하루에 세 개를 봤다.
  명단 대조 방식이라 그때마다 픽스처를 고쳐야 했다 — 에디터 한정 "정원 검사"가 근본 해결.
- 클론은 별개 프로세스라 **환경 전환이 따라오지 않는다**(메인만 `local`로 바꾸면 클론은 `local-k8s`로
  k8s 룸서버에 붙어 끊긴다).
