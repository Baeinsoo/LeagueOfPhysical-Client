...
- 서버: 받은 인풋을 인풋 버퍼(현 `InputBuffer` 컴포넌트)에 적재, 다음 서버 틱에서 1개씩 꺼내 처리
...
| (서버·공유) `LOP.InputBuffer` 컴포넌트 + `InputBufferSystem` (LeagueOfPhysical-Shared. 구 `EntityInputComponent` — 2026-07-02 입력-as-데이터로 데이터 컴포넌트+시스템 분리) | 인풋 버퍼(데이터) + 처리 로직 |
...
- Phase 2/3 도입 후 기존 `SnapReconciler`의 `entityTransformSnaps` 기반 delayed rendering (`SnapReconciler.cs:177-202`)과의 상호작용 확인
  - ✅ **해소 (2026-07-13) — 브래킷 탐색으로 근본 수정.** (박제 2026-06-28: 지연 렌더링이 스냅을 *틱 카운터*로 저장하고 `LateUpdate`가 *경과시간 역산*으로 **절대 틱 키**를 조회하던 취약점 — 둘이 어긋나면 `BoundedDictionary` 인덱서가 `KeyNotFoundException` → `[임시]` `TryGetValue` 스킵 가드로 *증상만* 차단.) 근본은 "시간 기준"이 아니라 *절대 키 조회 방식*이었다(시간 기준 보간 자체는 Fiedler 표준으로 옳음). **원격**(`ServerStateReconciler`→`RemoteEntityInterpolator`)은 07-07 표준 스냅샷 보간 재설계로 이미 해소, **로컬**(`SnapReconciler`→`LocalEntityInterpolator`)은 07-13 `GameFramework.Netcode.SnapshotInterpolation.Solve`(연속 renderTime을 감싸는 두 샘플을 **브래킷 탐색** + 범위 밖 hold → 절대 키 불필요, 미스 구조적 불가)로 교체. 시간 기준(alpha) 보간 유지. `[임시]` 가드 삭제됨.
