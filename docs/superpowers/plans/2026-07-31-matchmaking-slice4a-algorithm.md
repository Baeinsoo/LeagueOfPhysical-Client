...
> **정정(Task 3 리뷰 후):** 위 문구를 그대로 읽으면 *"다른 게임의 맵만 원한 티켓"* 이 교집합을 비워
> 폴백을 유발해야 할 것처럼 보인다. 그렇지 않다 — **그 상황은 정상 티켓에서 생길 수 없다.**
> spec §5대로 맵은 게임에 종속이라, 티켓은 **게임을 하나로 정했을 때만** 맵을 적을 수 있다
> (클라도 게임을 랜덤으로 두면 맵 선택을 숨긴다). 그러면 맵을 적은 티켓은 항상 그 한 게임의
> 제안에만 들어가므로 맵이 어긋날 수가 없다. 어긋난 티켓이 오면 그 소원만 조용히 무시한다 —
> 잘못된 데이터 하나 때문에 멀쩡한 매치를 죽이는 것보다 낫고, 그 불변식을 강제하는 자리는
> 티켓을 만드는 쪽(4b)이지 맵을 고르는 여기가 아니다.
