...
지금은 적이 **10초에 1마리씩, 상한 100마리**로만 늘고 줄일 방법이 없다. 같은 세션 안에서 조건을 바꿀 수 있어야 원인을 엔티티에 귀속시킬 수 있다.
...
Expected: 2초마다 한 줄, 유휴 상태에서 **`lag=0`**(또는 0/1을 오가는 정도)이고 `frameMaxMs`가 `budgetMs`보다 작다. 이게 건강한 기준선이다.
...
| 정상 | 증가·요동 | 증가 | **A — 서버 틱 밀림** (서버 로그 `lag`·`frameMaxMs`로 확증) |
