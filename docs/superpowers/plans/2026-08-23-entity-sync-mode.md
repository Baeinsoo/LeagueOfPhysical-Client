...
일곱 태스크를 서브에이전트로 실행했다. 계획과 달라진 것만 적는다 — 상세 결과·관측 수치·검증의 한계는
스펙 `2026-08-23-entity-sync-mode-design.md` §10-1에 있다.

| 저장소 | 커밋 |
|---|---|
| LOP-Shared | `cdd32ab` 모드·정책 + 테스트 5개 |
| LOP-Client | `a4bb23f` rename · `c396a55` 엔티티별 스무딩 · `d7283e8` 되감기 배치 · `5f9967b` 정책 배선 · `6d0309b` 임시 로그 제거 · `2401b22` 최종 리뷰 반영 |

**계획에서 달라진 것**

- **런타임 검증을 사람 손 없이 했다.** MPPM 클론이 `<project>/Library/VP/<id>` 아래 자기 파이프라인 포트를
  열어 두므로 `unity cmd <명령> --project-path <그 경로>`로 조작된다. 메인 에디터와 클론 두 클라를 CLI로
  몰아 2인 몸싸움까지 관측했다(계획은 "방법을 따로 궁리한다"로만 적어 두었다).
- **최종 리뷰가 계획의 판단 하나를 뒤집었다** — "정책을 두 곳에서 평가해도 두 게임에선 답이 갈리지
  않는다"가 틀렸다(같은 검증에서 관측된 `userEntityId` 경합이 그 전제를 깬다). 스냅 라우팅이 정책에 다시
  묻지 않고 `Simulated` 마커를 읽도록 고쳤다(`2401b22`).
- **보정 게이트**(스펙 Open Decision)는 유지하되 판정을 배치 전체로 넓혔다. 실측 약 44% 틱에서 열렸고
  평균 오차 0.01m였다.
- **미룬 것**: 정책의 집(공유 패키지 vs 클라) · `Reconciler` 자동 테스트 · `userEntityId` 스폰 경합(기존 버그).
