...
>
> **⟹ 실행 중 뒤집힘(2026-08-23, Task 5b)**: 실기 검증 뒤 사용자 결정으로 **카메라 드래그 배선을
> 통째로 제거**했다. 코스가 한 방향이라 시점을 돌릴 이유가 없고, 누르는 순간 플랩이 나가는 구조에서
> 드래그를 함께 두면 오조작만 남기 때문. 화면은 이제 날갯짓만 한다. Flappy 전용 고정 옆시점 카메라는
> 여전히 범위 밖이다.
...
계획대로 여덟 태스크를 서브에이전트로 실행했다. 계획과 달라진 것만 적는다 — 상세 결과는 스펙
`2026-08-17-flappy-race-gameplay-b2-design.md` §6-2 "결과"에 있다.

| 저장소 | 커밋 |
|---|---|
| GameFramework | `c09e5c9` 컴포넌트 · `9625db5` 높이≥지름 검사(최종 리뷰 반영) |
| LOP-Shared | `04a2032` 시뮬이 엔티티 몸을 읽는다 |
| LOP-Client | `370bc6f` 크리에이터·팔로워 · `f341557` `Simulated` · `e6106dd` 날갯짓 화면 · `1f7b2b3` 카메라 드래그 제거 · `8169cb7` 리뷰 반영 |
| LOP-Server | `2d17187` 크리에이터·팔로워 · `d364211` 리뷰 반영 |

**계획에서 달라진 것**

- **Task 5b 추가** — 실기 검증 뒤 사용자 결정으로 플랩 화면의 카메라 드래그 배선을 제거했다(위 Task 5 주석).
- **Task 7(MPPM 2인 몸싸움) 미검증 종결** — 가상 플레이어로 새 두 마리를 부딪히게 만들기 어려웠다.
  게다가 최종 리뷰에서 **클라가 몸싸움을 구조적으로 예측하지 않는다**는 것이 드러나(스펙 §6-2 "검증의
  한계"), 실기로 봐도 "예측 없음 + 보정"이 보일 자리다. 다음 슬라이스로 넘긴다.
- **유니티 명령 정정** — 계획서가 적은 `refresh_unity`는 이 CLI에 없고, 배치모드 `unity test`는 그
  프로젝트의 에디터가 떠 있으면 못 쓴다. 실행 중 실제 명령으로 고쳤다(커밋 `f2a105f`).
- **최종 리뷰 수정 웨이브** — 높이≥지름 검사 추가, `PhysicsFollower`의 fail-fast 검사를 맨 앞으로,
  `FlapPadView` 잔여물 정리, UXML/USS 파일 끝 개행.
- **미룬 것** — 디버그 HUD가 깔린 자리는 탭이 먹지 않는다(FlapWang에도 있는 기존 성질).

**검증에서 배운 배포 함정**: `gameserver-deploy` CI는 형제 패키지 레포를 *러너 자신의 클론*에서
`reset --hard @{u}`로 맞춘다. 그 클론은 main을 보고 있어 피처 브랜치의 패키지 변경이 CI에 안 보인다.
검증 때 러너 클론을 브랜치로 돌렸다면 **끝나고 main으로 되돌릴 것.**
