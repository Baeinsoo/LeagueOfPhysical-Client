...
- 간격 150m에서 대자로 갈 수 있는 거리는 약 **68m**, 다이브는 약 **33m**다. 표의 거리는 전부 68 이하이므로 대자로는 다 닿고, 40m를 넘는 4·5번은 **다이브로 곧장 가면 못 닿는다** — 자세를 고르게 만드는 것이 이 표의 목적이다.
...
namespace LOP.EditorTools
{
    ...
    public static class SkydiveCourseBuilder
    {
        ...
        private static readonly Shelf[] Shelves =
        {
            ...
            new Shelf(700f, 25f, 0f, 24f),
            new Shelf(550f, 25f, 25f, 20f),
            new Shelf(400f, -15f, 20f, 20f),   // 여기부터는 다이브로 곧장 가면 못 닿는다
            new Shelf(250f, -15f, -25f, 16f),
            new Shelf(100f, 20f, -20f, 16f),
        };
...
Expected: "통과 가능한 코스다." + 여섯 줄. 4·5번 선반 줄에 `(다이브로는 못 닿음 — 의도)`가 붙어 있어야 한다 — 그게 이 표의 존재 이유다.
...
2. 4·5번 선반은 **다이브로 곧장 가면 못 닿는다.** 대자로 펴서 옆으로 가야 한다. 이게 안 느껴지면 표의 거리를 늘린다.
