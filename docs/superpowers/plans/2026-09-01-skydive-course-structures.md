...
> **실행 중 바뀐 것 — `EditorUtility.DisplayDialog`를 전부 걷어냈다.**
> 위 코드 블록은 결과를 모달 대화상자로 띄우게 쓰여 있지만, 실행하면서 **모달 대화상자가 에디터
> 메인 스레드를 통째로 잡는다**는 것을 실제로 겪었다(이 슬라이스 도중 `Scene(s) Have Been Modified`
> 대화상자 하나 때문에 모든 에디터 명령이 60초 타임아웃 났다). 자동화로 메뉴를 부르면 누를 사람이
> 없어 영영 안 풀린다. 그래서 실제 코드는 성공이면 `Debug.Log`, 실패면 `Debug.LogError`만 쓴다 —
> 결과는 콘솔에서 읽는다. 이 파일의 `EditorUtility` 호출은 **따라 쓰지 말 것.**
...
- [ ] **C. Art 레포에서 씬만 커밋·푸시한다.** Art 레포에도 늘 로컬 픽스처(`.mat` 여러 개)가 있으니 `Scenes/SkydiveMap.unity`만 경로로 지정한다.
- [ ] **D. 클라 레포의 서브모듈 포인터를 새 Art 커밋으로 옮겨 커밋한다.**
      **서버 레포에는 Art 서브모듈이 없다**(실측 확인) — 서버는 맵을 어드레서블로만 받는다. 그래서 옮길 포인터는 클라 하나뿐이고, 서버에 새 맵을 주는 것은 아래 E다.
- [ ] **E. 클라 레포에서 `content-deploy`를 돌린다.** 맵은 어드레서블로 배달되므로 S3에 올라가야 서버가 새 씬을 받는다. **이걸 빼면 서버는 선반 없는 옛 맵을 계속 본다** — 클라 화면엔 선반이 보이는데 서버는 통과시켜 위치가 어긋난다.
