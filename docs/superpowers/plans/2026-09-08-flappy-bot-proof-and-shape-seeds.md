...
## 실행이 뒤집은 이 계획의 결정 (2026-09-10 기록)

이 계획을 그대로 따르면 안 된다 — 실행 중 **일곱 라운드의 리뷰**가 아래를 뒤집었다. 네 건은
코드에 "계획서대로 하지 말라"는 주석까지 붙어 있다. 근거 숫자는 모두 오프라인 재구현으로 쟀다
(실제 `.cs`를 `csc`로 컴파일해 `mono` 실행 + 완벽 오라클 대조).

| 계획서 | 실제 코드 | 왜 |
|---|---|---|
| "봇의 규칙은 한 줄이다 … 이보다 정교하게 만들지 않는다", `BotDecision.AimY`, `AimHeight` 소비 | 규칙 셋, `AimY` 없음, `AimHeight` **호출자 0** | 한 줄 규칙은 틈이 아치보다 좁을 때 바닥을 겨냥해 4m 아치로 천장을 뚫었다. 블렌딩은 **2593틱 판단 차이 0**으로 죽은 코드임이 실증돼 제거 |
| `if (state.Position.x + shape.Radius >= finishX)` | 발 기준(반지름 없음) | 몸 반지름을 두 번 세는 꼴 |
| `BotLookaheadSeconds = 0.14f` | `0.20f` | 0.14초는 형제 시스템(`FlappyAutoFlapSystem`)이 **"계속 박았다"고 기록하고 버린 값**이다 |
| `Func<float,float,bool> isBlocked`, `(x,y) => grid.IsFree(x,y) == false` | 이름 있는 델리게이트 **둘**을 `isFree` 극성으로, 타입이 박힌 **필드**로 | 극성이 뒤집힌 람다를 그대로 넘기는 사고를 컴파일 에러로 만든다. 델리게이트 *타입*만 갈라서는 부족했다 — 호출부가 **메서드 그룹**을 넘기는데 그건 어느 타입으로도 변환된다 |
| `seed = hit.point + hit.normal * 0.5f` | `hit.point.y - Height - 0.05f` (x 유지) | 법선 방향으로 0.5m 밀면 발 기준·높이 0.9인 몸의 **머리가 천장 위로** 나간다(0.40~0.75m). 즉 새를 바위 안에 놓는 시드였다 |
| 판정 표 세 가지(✅/🟡/❌) | **넷** — 봇 통과 ✅와 재생 증명 ✅가 다른 경로이고, 스폰이 지형 안이면 그 셋 어디에도 안 들어간다 | |

**계획서가 안 적은 것 하나 — 봇을 믿을 근거.** 이 도구의 판정이 뜻을 가지려면 봇이 **물리보다
겁이 많으면 안 된다**(그러면 "맵이 어렵다"가 "봇이 겁쟁이다"로 뒤바뀐다). 그 근거로 매 수정마다
**버틸 수 있는 가장 좁은 직선 회랑**을 쟀다 — 현재 **4.4225**(자유 발 구간 [0,W], 600틱, 시작
높이 전수, 0.0025 해상도). 완벽한 사람은 시작 높이에 따라 4.465~4.513이 필요하니 봇이 물리를
이긴 게 아니라 닿은 것이다. **이 숫자가 앞으로 이 봇을 고칠 때의 게이트다** — 어느 방향으로든
움직이면 그 자체가 발견이다.

**Task 6(문서 반영)은 이 계획 안에서 실행되지 않았다.** 브랜치 전체 리뷰가 잡아 2026-09-10에
따로 처리했다 — ROADMAP의 "①은 정직하게 미해결" 문단과 spec §3.1/§3.7/§6이 그 대상이었다.
