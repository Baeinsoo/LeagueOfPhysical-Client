...
> **이 태스크는 두 덩어리로 나뉜다.** Step 1~3·5는 로컬 작업이라 바로 한다. **Step 4(아트 main
> 머지·푸시)와 Step 6(두 클라 실측)은 사람 손이 필요하다** — 푸시는 공유 브랜치를 움직이는 일이고,
> MPPM 클론은 사람이 켜야 한다. 그 둘은 컨트롤러가 사용자와 함께 진행한다.
>
> **로컬 실측만 하려면 아트 포인터 커밋이 필요 없다** — 에디터는 서브모듈의 *워킹트리*를 본다.
> 포인터 커밋·푸시는 CI(`content-deploy`)가 맵을 보기 위한 것이라 브랜치를 닫을 때 한다.

- [ ] **Step 4: 아트 포인터를 커밋한다** *(사람과 함께 — 지금 하지 않는다)*
...
- [ ] **Step 6: 끝-끝으로 확인한다** *(사람과 함께 — 지금 하지 않는다)*
