...
- **판정에 관여하는 블록의 정의**: Default 레이어 **렌더러** 중 **z범위가 새의 z대역과 겹치는 것.** 배경(z 60~64)은 안 겹치므로 자동 제외된다
- **뒤쪽 두께 d** = `renderer.bounds.max.z − 0` (판정면보다 뒤로 뻗은 양 — **재는 것은 렌더러의 범위이지 콜라이더가 아니다.** 최종 설계(Task 3 머리말 참고)는 콜라이더를 아예 옮기지 않으므로 콜라이더 쪽을 재도 왜곡을 볼 수 없다)
- 왜곡 식: `파고드는 양 = h · d/(C+d)`
...
> **⚠️ 구현 중 뒤집힌 결정 (2026-09-15).** 아래 Step 1의 코드는 원래 설계 — "블록(콜라이더
> 포함)을 제 두께만큼 카메라 쪽으로 당긴다" — 를 그대로 옮긴 것이다. 실제로 그렇게
> 구현했다가(Art 커밋 `3ac9cba`) 리뷰에서 결함이 나와 **폐기했다.** 실제로 실린 설계(Art
> 커밋 `8c332be`)는 **렌더러만 옮기고 콜라이더는 원래 자리에 그대로 둔다** — 아래 Step 1
> 코드는 그 최종 형태가 아니다. 자세한 설계는 spec §3①을 참고할 것. 이 문서를 그대로
> 따라 콜라이더를 옮기지 말 것.
>
...
>
> 실제로 구현된 코드는 `Assets/Scripts/Editor/FlappyDepthAlignTool.cs`를 참고한다 —
> 렌더러를 옮기고 `BoxCollider.center`를 반대로 보정해 콜라이더 월드 bounds를 고정한다.
> z는 정확히 불변, x/y는 4µm 이내(회전 부모를 거치는 float 왕복 잔차 — 게임플레이 영향 없음).
...
| §7 z=1.4의 4개 | **정정(2026-09-15) — 애초에 대상이 아니었다.** 이 4개는 `Halo` 장식이 x축으로 90° 회전된 것으로, 로컬 스케일의 0.08m 두께 쪽 축이 z로 눕는다. 실제 월드 z 범위는 `[1.36, 1.44]`로 새의 z대역 `[-0.45, +0.45]` **밖**이라 판정에 안 걸리고 콜라이더도 없다. "가장 두꺼운 2.7m"는 회전을 안 빼고 로컬 트랜스폼 행만 읽어 나온 잘못된 값이었다(spec §7 참고) |
