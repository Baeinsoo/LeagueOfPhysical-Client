...
> **⚠️ 실행 중 교정 (2026-09-20).** 아래 규칙 중 *"게임 평면인데 콜라이더가 없으면 위반"* 은
> **너무 엄격해서 폐기했다.** 실제 씬에 걸었더니 위반 26개가 나왔는데 전부 소품과 마커였다
> (`Coin` 여럿, `FinishLine` 배너 — 콜라이더가 없는 게 정상인 것들). 게임 평면 높이에 있다는
> 것만으로 장애물이라고 보면 검사가 잡음만 뱉고 쓸모없어진다.
>
> **고친 기준: 콜라이더가 아니라 재질이 장애물인지를 정한다.**
>
> | 경우 | 판정 |
> |---|---|
> | 배경 + 콜라이더 | ❌ 안 보이는 벽 |
> | 게임 평면 + 콜라이더 + 배경 재질 | ❌ 장애물인데 배경처럼 읽힌다 |
> | 게임 평면 + **장애물 재질** + 콜라이더 없음 | ❌ 칠해 놓고 통과된다 |
> | 게임 평면 + 소품 재질 + 콜라이더 없음 | ✅ 배너·동전 — 정상 |
>
> 잡으려던 버그 둘은 그대로 잡고 오탐만 없앤다. 아래 코드와 테스트는 이 교정이 반영된 것이
> 실제로 커밋됐다(테스트 10개).
