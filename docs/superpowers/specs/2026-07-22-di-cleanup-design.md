...
- GameFramework/Shared **패키지**엔 `[Inject]`가 **0개**(공유 코어는 필드 주입을 안 쓴다). 단 클라·서버 **프로젝트**의 Assembly-CSharp엔 각각 있다.
- 클라에서 `[Inject]`를 쓰는 파일은 35개, **서버는 21개 파일·67곳**(MonoBehaviour + 순수 C# 혼재). 두 프로젝트는 같은 이름의 클래스를 다수 공유한다(`EntitySpawner`/`CharacterCreator`/`ItemCreator`/`EntityBinder`/`AbilityActivator`/메시지 핸들러/데이터 프로바이더). 따라서 생성자 전환은 **클·서 양쪽 프로젝트** 작업이다(공유 패키지는 무영향).
...
**클라·서버 두 프로젝트의 순수 C# 클래스.** 파일별로 MonoBehaviour 상속 여부를 확인해 순수 C#만 전환한다. 후보(최종은 구현 시 확정):
...
서버(21개 파일):
- 클라와 공유하는 이름 — `GameInfoMessageHandler`, `GameEntityMessageHandler`, `GameInputMessageHandler`, `EntityBinder`, `EntitySpawner`, `CharacterCreator`, `ItemCreator`, `AbilityActivator`, `AbilityDataProvider`, `StatusEffectDataProvider`
- 서버 전용 — `LOPAIController`, `ConfigureRoomComponent`, `CombatConfigProvider`, `LoadMasterDataComponent` 등(순수 C#인 것만)

> **서버 로컬 픽스처 주의:** 서버 working-tree에는 커밋하면 안 되는 로컬 테스트 픽스처(`ConfigureRoomComponent`, `LOPGame` 등)가 있다. `ConfigureRoomComponent`는 전환 대상과 겹치므로, 서버 커밋 시 픽스처성 로컬 변경을 섞지 않도록 diff를 확인하고 필요하면 stash로 분리한다.
...
생성자 주입으로 바뀌면 `new`로 인스턴스화가 가능해진다. 의존성이 인터페이스/스텁 가능한 클래스는 EditMode 단위 테스트를 추가한다. 단, 클라·서버 프로젝트 코드는 Assembly-CSharp라 asmdef가 참조할 수 없으므로, 테스트가 가능한 것은 순수 로직이 패키지(GameFramework/Shared)에 있는 경우로 제한된다. 프로젝트 Assembly-CSharp에 묶인 클래스는 단위 테스트 대상에서 제외하고 전환만 한다.
