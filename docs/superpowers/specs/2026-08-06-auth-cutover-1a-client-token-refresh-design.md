...
1. **회귀 없음** — 로그인 → 로비 진입 → 매칭 요청/취소가 이전과 동일.
2. **미리 갱신 + 합치기** — `AccessTokenInfo.DefaultRefreshMargin`을 일시적으로 크게(예: 59분) 바꾸면
   토큰 수명이 1시간이라 **모든 요청이 갱신을 유발**하게 된다. 이 상태로 로비에 진입한다(유저 정보·전적·
   위치 요청이 한꺼번에 나가는 순간이다). **그 한 순간에 로그인 호출이 1번만 찍히면 통과** — 합치기가
   없으면 동시 요청 수만큼 찍힌다. 확인 후 값 복구.
