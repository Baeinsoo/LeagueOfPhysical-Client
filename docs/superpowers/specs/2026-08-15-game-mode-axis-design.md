...
| **C** | 로비 게임 선택 | 클라 UI | ✅ **완료.** 로비에서 고른 게임으로 실제 입장한다 (아래 "슬라이스 C 결과") |
...
- [x] ~~**`MatchmakingRequest`의 `gameModeId` 단수 ↔ 백엔드 `gameModeIds[]` 복수**~~ **해소(2026-08-17,
  슬라이스 C) — 바꿀 것이 없었다.** 백엔드 HTTP DTO(`matchmaking.dto.ts`)가 단수 `gameModeId`를 받아
  티켓을 만들 때 `[gameModeId]`로 감싼다(`matchmaking.service.ts`). 클라 DTO는 그대로 두면 된다.
...
## 슬라이스 C 결과 (2026-08-17)

**완료.** 로비에서 게임과 맵을 골라 그대로 입장한다. `MatchmakingViewModel`의
`TemporaryGameModeId`/`TemporaryMapId` 하드코딩은 삭제됐다.

### 스펙과 달라진 것 (그리고 왜)

| 스펙이 말한 것 | 실제로 한 것 | 왜 |
|---|---|---|
| 게임 2개를 나란히 놓은 **카드** | **PLAY 버튼 안에 드롭다운 2개**(위=게임 / 아래=맵) | 사용자 선택. 카드를 늘어놓으면 게임이 늘 때 로비가 붐비고, 맵까지 고르려면 자리가 더 필요하다 |
| 게임만 고른다 (맵은 범위 밖) | **맵도 고른다** | 게임당 맵이 여러 개가 될 때를 대비해 자리를 잡아 뒀다. 지금은 게임당 맵이 하나라 항목이 하나씩만 뜬다 |
| `TbGameMode.Name`/`Description`이 "이미 있는 컬럼" | 이름은 이미 채워져 있었고 **설명은 FlappyRace만** 있다 | 카드/드롭다운은 이름만 쓰므로 마스터데이터 작업이 아예 없었다(3레포 커밋 절약) |

### 목록에 무엇을 보여줄지 — 데이터가 정한다

`TbGameMode` 6행 중 실제로 입장 가능한 것만 뽑는다(`PlayableGameProvider`):

```
씬 경로가 있다  AND  그 게임에 속한 맵이 하나 이상 있다
```

둘 다 "없으면 골라도 못 들어가는" 전제조건이다 — 씬 경로가 비면 `MatchSceneResolver`가 예외를
던지고, 맵이 없으면 티켓에 실을 `mapId`가 없다. 아직 만들지 않은 4개(`Dodgeball`·`ObserverAvoid`·
`RememberGame`·`TargetShooting`)는 **씬 경로가 비어 있어 저절로 빠진다.** 더미 행을 지우지 않아도
되고, 앞으로 새 게임 행이 씬보다 먼저 들어와도 로비에 새지 않는다.

### 실제로 걸린 함정 (전부 런타임에서 드러난 것)

- **`strings`는 UTF-8 한글을 안 보여준다.** `.bytes`를 `strings`로 훑고 "이름 컬럼이 비었다"고
  잘못 판단했다. 마스터데이터 확인은 `strings` 말고 UTF-8로 디코드해서 봐야 한다.
- **`--lop-radius-pill`(999px)을 비정사각 요소에 주면 모서리가 타원이 된다.** 유니티는 반지름이
  요소보다 클 때 가로·세로를 *각각* 줄인다(CSS 표준은 한 비율로 함께 줄여 알약이 된다).
  400×84 필이 실제로 타원으로 깨졌다. **높이의 절반을 직접 적어야 한다.** 게임패드의 조이스틱·
  액션 버튼은 정사각형이라 같은 토큰을 써도 멀쩡하다.
- **`-unity-text-outline-width`는 상속된다.** 드롭다운을 `.btn` 안에 넣자 부모의 3px 주황
  외곽선이 글자에 내려와 지저분해졌다. 자식에서 `0`으로 꺼야 한다.
- **드롭다운 펼침 목록은 UIDocument 밖(패널 루트)에 붙는다.** 화면별 USS로는 절대 안 잡히고
  **테마(`Theme.uss`)** 에 둬야 먹는다. 그래서 이 스타일은 프로젝트 전체 드롭다운에 적용된다.
- **유니티는 "어느 항목이 선택됐는지"를 클래스로 알려주지 않는다** — 체크마크의 `visibility`로만
  표시한다. USS엔 "자식이 보이는 부모"를 고르는 문법이 없어 코드가 클래스를 붙여야 한다.
  판정은 **인덱스**로 한다(항목은 `choices`와 같은 순서). `visibility`는 메뉴가 갓 열린 프레임에
  확정되지 않아 전부 선택된 것처럼 보이고, 이름 비교는 이름이 겹치면 깨진다.
- **버튼 안에 버튼을 넣으면 클릭이 샌다.** 드롭다운을 여는 클릭이 바깥 PLAY까지 올라가면 매칭이
  시작된다. 자식에서 `PointerDown`/`PointerUp`/`Click` 전파를 끊어야 한다.
- **UI Toolkit 스크린 오버레이는 스크린샷에 안 잡힌다**(`capture_game_view`·`screenshot` 모두).
  모양 확인은 사람 눈이 필요하고, 구조·상태는 `eval`로 트리를 훑어 단언하는 게 확실하다.

### 런타임 검증

`local-k8s`(게임서버=k8s 파드) 실제 경로로 확인했다.

| 확인 | 결과 |
|---|---|
| 목록 | `[플랩왕, 플래피 레이스]` — 미구현 4개는 자동 제외 |
| 게임을 바꾸면 | 맵 목록이 그 게임의 맵으로 교체되고 첫 맵이 선택됨 |
| 플래피 레이스 → PLAY | `Room + FlappyRace + FlappyRaceMap` 로드, 엔티티 스폰, 클라 콘솔 에러 0 |
| 서버가 받은 값 | DB `MatchRound`: `gameModeId=6, mapId=2` |
| 플랩왕 → PLAY | 티켓 `gameModeIds={1}, mapIds={1}`로 수락되어 대기 (FlapWang은 `min_players=2`라 혼자선 매치 성립 불가 — 완주 확인은 2번째 클라 필요) |
