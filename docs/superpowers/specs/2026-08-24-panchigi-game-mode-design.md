...
#### 결정 — "누가 진실원본인가"를 엔티티가 들고 있게 한다

지금 코드에는 두 종류뿐이다. 판치기가 세 번째를 만든다.

| 엔티티 | 누가 굴리나 | 방향 |
|---|---|---|
| 내 캐릭터·새 (`Simulated`) | 우리 시뮬 | World → 몸 (push) |
| 클라의 남·NPC (표식 없음) | 스냅샷 보간 | World → 몸 (push) |
| **판치기 동전** (신규) | **PhysX** | **몸 → World (읽기)** |

**`PhysicsDriven` 마커 컴포넌트**를 둔다. 이게 붙은 엔티티는 `PhysicsSimulationSystem`이

1. `PushMotion`을 **건너뛰고**
2. `Simulate` 뒤에 **rb → World로 읽어온다**

`Simulated`(우리 시뮬이 굴린다)과 짝이 되는 표식이라, 둘 다 없으면 보간이 굴린다는 3분할이 코드에
드러난다. 인터페이스가 아니라 **데이터**이므로 게임 스코프에 등록할 것은 없다(§4와 같은 원리).

```
PushMotion 건너뜀 → PhysX 스텝(동전이 굴러감) → rb → World.Transform
                                                        ↓
                                    EntitySnapshotBroadcastSystem (기존 그대로)
```
...
동전은 **원반 + 회전 자유 + 다이나믹**인데, 지금 클·서 `EntityBinder`는 몸 만들기를 하드코딩한다:
...
**서버 전용**이라서다. 몸 만들기는 클·서가 같아야 하는 것이라 축이 다르다.

### 4.2 결정 — shape 컴포넌트를 형제로 늘린다

몸 치수는 이미 데이터다. `CapsuleShape`의 주석이 *"값은 게임이 정한다(엔티티를 만드는 쪽이 붙인다)"*
라고 명시한다. 그 원칙을 그대로 늘린다.

```
게임이 정하는 것 = 데이터              만드는 코드 = 클·서 공유 구체 하나
──────────────────────────            ──────────────────────────────────
FlappyBirdCreator → CapsuleShape ┐
                                 ├──→  PhysicsBodyFactory  →  몸
PanchigiCoinCreator → DiscShape  ┘      (붙어 있는 shape을 보고 고른다)
```

- **`DiscShape`을 `CapsuleShape` 형제로 World Core(`GameFramework.World`)에 추가.** 몸 치수는 앱
  비종속이고 이미 있는 것과 대칭이다.
- **`PhysicsBodyFactory`는 하나로 유지.** 붙어 있는 shape을 보고 콜라이더를 고른다.
- `isKinematic`/`isTrigger`도 지금은 바인더가 `EntityKind`로 하드코딩하는데(`isItem` 분기), 같은
  원리로 데이터가 되는 게 맞다. 동전은 `PhysicsDriven`(§3.5)이 곧 "다이나믹"을 뜻하므로 그 표식에서
  자연스럽게 온다.
...
| **2** | `DiscShape` + `PhysicsDriven` + 동전 스폰 + PhysX→World 되읽기 | 동전이 떨어져 쌓이는 게 **두 클라 모두에** 똑같이 보인다 |
...
- **몸 치수를 데이터로 두고 팩토리는 하나** — 클·서 공유 시뮬이 같은 몸을 보게 하는 표준 배치.
