...
| **나머지 등수** | 공동 꼴등 |
| **장외** | 나간 동전만 **초기 세팅**(스폰 위치·시작 면)으로 복귀 |
...
PhysX 스텝 (동전이 굴러감)
      ↓
동전 rb 위치·회전  →  World.Transform      ← 새로 만드는 틱 시스템
      ↓
...
**게임 스코프에 새로 등록할 것은 없다.** 크리에이터가 붙이는 컴포넌트가 하나 늘 뿐이다.
...
| `TbPanchigiConfig` | id = 1 단일 행 → 세기 상한 · 정지 문턱 · 조준 제한시간 · 매치 상한 |
...
1. 장외로 나간 동전은 **초기 세팅**(스폰 위치·시작 면)으로 복귀
2. 각 동전의 면을 읽는다 — 업벡터가 위를 보면 앞, 아래면 뒤
3. **전부 시작 면의 반대**면 → 방금 친 사람이 승자, 나머지 공동 꼴등
4. 아니면 다음 사람으로 턴 넘김
...
| **2** | `DiscShape` + 동전 스폰 + PhysX→World 되읽기 | 동전이 떨어져 쌓이는 게 **두 클라 모두에** 똑같이 보인다 |
| **3** | 타격 입력 + 힘 커널(원본 물리 재작업) | 칠 수 있고 동전이 튄다 |
| **4** | 턴 상태 기계 + 면·종료 판정 | **게임이 된다** — 이기고 진다 |
...
- **동전 수·대형의 실제 숫자** — 붙여보고 튜닝. 테이블 자리는 §6에서 잡는다
