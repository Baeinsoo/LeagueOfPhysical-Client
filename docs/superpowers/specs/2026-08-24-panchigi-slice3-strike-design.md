...
K개 샘플은 **동전 발자국 원 위에 결정론적으로** 깐다 — 중심 1개 + 동심원 링. 난수를 쓰지 않는다
(같은 타격이 같은 결과를 내야 하고, 클라가 나중에 미리보기를 붙일 여지를 남긴다).

```
K = 13: 중심 1 + 반지름 0.5r 링 4 + 반지름 1.0r 링 8
```

`CoverageSamples`는 이 패턴의 총 개수이고, 패턴 자체는 코드가 정한다(테이블은 개수만 고른다).
지원하지 않는 K가 들어오면 **예외를 던진다** — 조용히 가까운 값으로 바꾸면 테이블 값과 실제
동작이 어긋난 채 굴러간다.
...
| 같은 입력 두 번 | 완전히 같은 결과 |
| `BuildSamples` | K개 전부 발자국 원 안, 같은 입력이면 같은 배치 |
| 지원하지 않는 K | 예외 |
