...
| 턴 수가 `match_turn_limit`에 **도달** | **전원 무승부**(동일 등수) |

턴 수는 **친 것과 패스한 것을 모두** 센다 — 안 그러면 전원이 계속 패스해 무한히 늘어진다.

> **상한은 "도달"이지 "초과"가 아니다.** `match_turn_limit=60`이면 60턴을 돌고 끝난다.
> 처음엔 `TurnCount > limit`으로 적었다가 **61턴이 도는 것을 리뷰가 잡았다** — 노브 이름이
> 값과 어긋나면 나중에 튜닝하는 사람이 60을 넣고 61을 받는다.
>
> 반면 **승리 판정이 상한 판정보다 먼저 오는 순서는 의도된 것**이다. 정당하게 시작된 턴이
> 승리로 끝났으면 승자를 인정한다 — 치게 해 놓고 "사실 이미 끝난 판이었다"며 무승부로
> 만드는 쪽이 더 나쁘다.
...
### 대형은 이름마다 자기 자리 목록을 갖는다

`TbPanchigiSetup`은 2·3·4인 세 행을 갖고 있고 대형은 `FourInLine` / `SixInLine` / `FourByTwo`다.
**이름이 곧 모양이므로 자리 목록도 대형마다 따로 둔다** — 한 줄로 8개를 깔아 놓고 앞에서부터
잘라 쓰면 `SixInLine`이 "4개 줄 + 2개"가 되어 이름이 거짓말을 한다.

> **박제 — 내가 "4인 행이 없다"고 잘못 적었다.** 표를 확인하는 스크립트에 행 제한(`[:6]`)을
> 걸어 7번째 행을 스스로 잘라내고는, 그걸 근거로 없는 구멍을 메우는 절을 썼다. 그 지시대로
> 구현자가 `FourByTwo`를 `EightInLine`으로 개명했고(다른 모양이다), 구현자가 원본을 확인해
> 되돌렸다. **검증 자체에서 낸 오류**라 `[[verify-backlog-claims-before-working]]`의 반복이다.
