...
### 화면에 그리는 시각 — 한 시간대로 모은다 ★

**로컬에서 결정론적으로 도는 것(문·레이저 같은 장애물, NPC)은 *내 캐릭터를 그리는 시각과 같은
시각*으로 그린다.** 화면 안의 것들이 서로 다른 시각에 그려지면 몸과 장애물의 *간격*이 실제와
달라 보인다 — 판정은 멀쩡한데 눈에는 틀리게 보이는, 고치기 전엔 원인을 짚기 어려운 종류의 버그다.

시뮬은 정수 틱으로만 돌지만 화면은 그보다 자주 그려지므로, 뷰는 틱 사이를 담은 **소수 틱**을 쓴다.
기준값은 하나다:

```
renderTick = (tickUpdater.elapsedTime - tickUpdater.interval) / tickUpdater.interval
```

- **왜 한 틱 뒤인가**: 내 캐릭터 보간(`PredictedEntityInterpolator`)이 그 시각을 쓴다. 보간하려면
  앞뒤 샘플이 둘 있어야 하는데 "지금"에 서면 **앞쪽 샘플이 아직 계산되지 않았다.**
- **⛔ `tickUpdater.tick`을 뷰에서 쓰지 말 것.** 틱 루프는 `onTick(tick)` 뒤에 `tick++` 하므로,
  뷰가 도는 시점의 `tick`은 *방금 계산한 틱 + 1* — 아직 일어나지 않은 틱이다. 그대로 쓰면 장애물이
  몸보다 두 틱 앞서 그려진다. (2026-09-07 이전 `SkydiveLaserView`가 그랬다.)
- **연속과 이산을 가른다**: *자세*(위치·각도)는 소수 틱으로 물어 부드럽게, *상태*(빔 점등 같은
  켜짐/꺼짐)는 `floor(renderTick)`의 정수 틱으로 묻는다. 켜짐/꺼짐은 틱 단위 사실이라 소수로 물을
  것이 없다.
- **판정은 이 값을 절대 안 본다.** 뷰가 옮긴 트랜스폼에 콜라이더가 달려 있어도, 시뮬이 매 틱 첫
  줄에서 정수 틱 자세를 다시 세우고 `SyncTransforms`까지 한 뒤에 질의를 시작한다(스펙상 순서).
  이 순서가 깨지면 클라에만 있는 소수 틱 자세를 판정이 보게 되어 클·서가 갈린다.

> 원격 엔티티는 이 규칙 밖이다 — 서버 스냅샷을 받아 **자기 보간 시각**으로 그린다
> (`RemoteEntityInterpolator`). 여기서 말하는 것은 *로컬에서 같은 식으로 다시 계산되는* 것들이다.
...
- **`*System` = 무상태 DI 인스턴스**(월드/컴포넌트를 조작; 상태는 컴포넌트에 둔다). 다른 `*System`과 동일하게 DI 등록·주입한다. **`static`은 컨텍스트 없는 순수 커널에만** 쓴다(예: `MovementMotor.CalcVelocity`, `KinematicMover.Move`, `AbilitySystem.HasActiveMotionEffect`) — 그런 순수 커널엔 `*System` 이름을 붙이지 않는다(static 클래스를 "System"이라 부르지 않음). 업계 표준 정합: Photon Quantum도 *systems=인스턴스* / *`FPMath`=static util*로 가른다.
...
- **Slice 4(a~e) — Engine ↔ Simulation 추출**: 호스트(`LOPRunner`, 구 `LOPGameEngine`)의 시뮬 흐름을 공유 시뮬(`LOPWorld`, 구 `LOPGameSimulation`)로 점진 흡수. *(Runner/World 리네임 완료.)*
  ...
  - **4c (완료)**: 이동 커널 공유(`MovementMotor.CalcVelocity` — 당시 이름은 `MovementSystem.ProcessMovement`) + 어빌리티/StatusEffect 틱을 `LOPWorld.Mutation`으로 이전.
